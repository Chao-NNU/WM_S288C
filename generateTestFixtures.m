...
for  i = 1:numel(data)
    ...
    fwrite(fid, edu.jiangnan.fmme.io.jsonFormat(tmp));
    fclose(fid);
end
...
for i = 1:length(simulation.states)
    edu.jiangnan.fmme.cell.sim.CellStateFixture.store(simulation.states{i});
end

%% write process test fixtures
for i = 1:length(simulation.processes)
    edu.jiangnan.fmme.cell.sim.ProcessFixture.store(simulation.processes{i});
end

%% reset mass reference in geometry state, and regenerate fixtures
simulation.state('Geometry').mass = mass;
edu.jiangnan.fmme.cell.sim.CellStateFixture.store(simulation.state('Geometry'));
edu.jiangnan.fmme.cell.sim.ProcessFixture.store(simulation.process('Metabolism'));
