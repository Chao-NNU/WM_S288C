classdef KnowledgeBaseUtil_Test < TestCase
    ...
    methods
        ...
        function setUp(this)
            this.database = edu.jiangnan.fmme.db.MySQLDatabase(config);
        end
    ...
    methods (Static)
        function simWID = saveSimulation(kbWID, database)
            ...
            import edu.jiangnan.fmme.cell.sim.util.ConditionSet;
            import edu.jiangnan.fmme.cell.sim.util.DatabaseLogger;
            
            sim = SimulationFixture.load([], true);
            sim.applyOptions('lengthSec', 10);
            
            metadata = ConditionSet.parseConditionSet(sim, 'src_test/+edu/+jiangnan/+fmme/+cell/+sim/+util/fixtures/runSimulations.xml').metadata;
            metadata.knowledgeBaseWID = kbWID;
            metadata.revision = edu.jiangnan.fmme.util.revision;
