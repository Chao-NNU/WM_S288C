...
classdef ProteinGrowth_Test < TestCase
    ...
    methods
        function testExponentialGrowth(this)
            ...
            %% references
            sim = edu.jiangnan.fmme.cell.sim.SimulationFixture.load([], {
                'Metabolism'
        ...
        function testExponentialGrowthDistribution(this)
            ...
            %% plot
            %[axesHandle, figHandle] = edu.jiangnan.fmme.cell.sim.util.PlotUtil.newAxesHandle();
            %
            %cla(axesHandle);
    ...
    methods (Static = true)
        function fitGrowthRate()
            ...
            [macromoleculeStateInitializationGrowthFactor, ~, exitFlag, output] = edu.jiangnan.fmme.util.ComputationUtil.fzero(...
                @(x) edu.jiangnan.fmme.cell.sim.ProteinGrowth_Test.fitGrowthRate_diff(...
        ...
        function value = fitGrowthRate_diff(nIter, iterMax, stepSizeSec, metStepSizeSec, initSeed, seed, macromoleculeStateInitializationGrowthFactor)
            ...
            fitter.run();
            edu.jiangnan.fmme.cell.sim.SimulationFixture.store(sim, fixtureFileName);
            clear sim fitter ans;
            
            [~, growths] = edu.jiangnan.fmme.cell.sim.ProteinGrowth_Test.calcExponentialGrowthDistribution(...
        ...
        function fitInitialProteinCopyNumberVariance()
            ...
            [macromoleculeStateInitializationVariation, ~, exitFlag, output] = edu.jiangnan.fmme.util.ComputationUtil.fzero(...
                @(x) edu.jiangnan.fmme.cell.sim.ProteinGrowth_Test.fitInitialProteinCopyNumberVariance_diff(...
                nIter, iterMax, stepSizeSec, metStepSizeSec, initSeed, seed, x), ...
        ...
        function value = fitInitialProteinCopyNumberVariance_diff(nIter, iterMax, stepSizeSec, metStepSizeSec, initSeed, seed, macromoleculeStateInitializationVariation)
            [initCorr, divCorr] = edu.jiangnan.fmme.cell.sim.ProteinGrowth_Test.calcProteinCopyNumberCorrelationDistribution(...
                nIter, iterMax, stepSizeSec, metStepSizeSec, initSeed, seed, ...
        ...
        function [initCorr, divCorr] = calcProteinCopyNumberCorrelationDistribution(...
            ...
            [~, ~, ~, matureMonCounts, matureCpxCounts, ~, initMatureMonCounts, initMatureCpxCounts] = ...
                edu.jiangnan.fmme.cell.sim.ProteinGrowth_Test.calcExponentialGrowthDistribution(...
            ...
            for i = 1:nIter
                ...
                if isfinite(metStepSizeSec)
                    [initGrowths(i), ~, growths(i, :), ~, ~, ~, ~, ...
                        ...
                        initMatureRNACounts(:, i), initMatureMonCounts(:, i), initMatureCpxCounts(:, i)] = edu.jiangnan.fmme.cell.sim.ProteinGrowth_Test.simulateExponentialGrowth(...
        ...
        function [initGrowth, initMetabolicEnzymes, growths, cumGrowth, rnaExp, pmExp, pcExp, ...
            ...
            sim = edu.jiangnan.fmme.cell.sim.SimulationFixture.load(fixtureFileName, {
                'Metabolism'
                'MacromolecularComplexation'
                });
            ...
            %% constants
            rnaProd = edu.jiangnan.fmme.util.ComputationUtil.invertCompositionMatrix(rna.nascentRNAMatureRNAComposition) * ...
            ...
            while true
                if initSeed
                    edu.jiangnan.fmme.cell.sim.ProteinGrowth_Test.seedRandStream(sim, initSeed);
                end
            ...
            if seed
                edu.jiangnan.fmme.cell.sim.ProteinGrowth_Test.seedRandStream(sim, seed);
            end
