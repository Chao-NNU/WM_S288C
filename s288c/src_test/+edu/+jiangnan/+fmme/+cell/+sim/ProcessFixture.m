...
classdef ProcessFixture
    ...
    properties (Constant)
        pathPrefix = edu.jiangnan.fmme.cell.sim.ProcessFixture.determinePathPrefix();
    end

    methods (Static)
        function store(process, filename)
            import edu.jiangnan.fmme.cell.sim.ProcessFixture;
        ...
        function process = load(process, filename, loadStates)
            ...
            if ~loadStates
                ...
                for i = 1:numel(metaClass.Properties)
                    ...
                    if ~metaClass.Properties{i}.Dependent && isa(process.(name), 'edu.jiangnan.fmme.cell.sim.CellState')
                        process.(name) = [];
                    end
                end
            end
    ...
    methods (Static, Access = private)
        function value = determinePathPrefix()
            loc = which('edu.jiangnan.fmme.cell.sim.ProcessFixture');
