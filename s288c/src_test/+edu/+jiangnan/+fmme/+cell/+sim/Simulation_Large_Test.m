...
classdef Simulation_Large_Test < TestCase
    ...
    methods
        function testRNAAndProteinSynthesisAndDecay(this)
            ...
            metNucComp = edu.jiangnan.fmme.cell.sim.util.FitConstants_Test.getNucleotideCompositionMatrix(m);
            rnaNucBaseComp = (rna.baseCounts * edu.jiangnan.fmme.cell.sim.util.FitConstants_Test.getNucleotideCompositionMatrix(m)')';
            monNucBaseComp = (pm.baseCounts * edu.jiangnan.fmme.cell.sim.util.FitConstants_Test.getNucleotideCompositionMatrix(m)')';
            cpxNucBaseComp = (pc.baseCounts * edu.jiangnan.fmme.cell.sim.util.FitConstants_Test.getNucleotideCompositionMatrix(m)')';
            ...
            rnas(setdiff(1:end, rna.matureMRNAIndexs)) = ...
                rnas(setdiff(1:end, rna.matureMRNAIndexs)) + ...
                sum(pc.proteinComplexComposition(setdiff(1:end, g.mRNAIndexs), :, :), 3) * sum(pc.counts(pc.matureIndexs, :) + pc.counts(pc.boundIndexs, :), 2);
            assertElementsAlmostEqual(rnas' * rna.molecularWeights(rna.aminoacylatedIndexs) / edu.jiangnan.fmme.util.ConstantUtil.nAvogadro, ...
