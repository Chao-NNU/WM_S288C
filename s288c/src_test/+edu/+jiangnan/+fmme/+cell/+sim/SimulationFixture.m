...
classdef SimulationFixture
    properties (Constant)
        pathPrefix = edu.jiangnan.fmme.cell.sim.SimulationFixture.determinePathPrefix();
        defaultProps = edu.jiangnan.fmme.cell.sim.SimulationFixture.determineDefaultPropertyNames();
        defaultFilename = 'Simulation.mat';
    end

    methods (Static)
        function store(sim, filename)
            import edu.jiangnan.fmme.cell.sim.SimulationFixture;
            ...
            edu.jiangnan.fmme.io.editMatFileHeader(filename);
        end
            ...
            if exist('processes', 'var')                    
                ...
                for i = 1:numel(metaClass.Properties)
                    if ...
                            ...
                            isa(sim.(metaClass.Properties{i}.Name), 'edu.jiangnan.fmme.cell.sim.Process')
    ...
    methods (Static, Access = private)
        function value = determinePathPrefix()
            loc = which('edu.jiangnan.fmme.cell.sim.SimulationFixture');
            i = strfind(loc, filesep);
            value = [loc(1:i(end)) 'fixtures' filesep];
        end

        function value = determineDefaultPropertyNames()
            metaData = metaclass(edu.jiangnan.fmme.cell.sim.Simulation);
