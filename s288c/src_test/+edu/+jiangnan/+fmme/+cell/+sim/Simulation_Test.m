...
classdef Simulation_Test < TestCase
    ...
    methods
        ...
        function testRunLogging(this)
            ...
            %1 logger
            logger = edu.jiangnan.fmme.cell.sim.util.SummaryLogger(2, 0);
            sim.run(logger);
            
            %2 loggers
            logger1 = edu.jiangnan.fmme.cell.sim.util.SummaryLogger(2, 0);
            logger2 = edu.jiangnan.fmme.cell.sim.util.SummaryLogger(2, 0);
    ...
    methods (Access = private)
        ...
        function result = createMetadata(this)
            ...
            [result.userName, result.hostName, result.ipAddress] = ...
                edu.jiangnan.fmme.util.computerInfo();
            result.revision = 234;
        ...
        function assertPropertyValuesEqual(...
            ...
            for i = 1:length(sim.processes)
                ...
                for j = 1:length(timeCourseNames)
                    ...
                    if isa(timeCourses.(sim.processWholeCellModelIDs{i}).(name), 'edu.jiangnan.fmme.util.CircularSparseMat')
                        assertTrue(...
    ...
    methods
        ...
        function testInitializeFromDaughter(~)
            ...
            %run simulation
            daughter = runSimulation(tmpDirName, true);
