...
classdef CellStateFixture
    ...
    properties (Constant)
        pathPrefix = edu.jiangnan.fmme.cell.sim.CellStateFixture.determinePathPrefix();
    end

    methods (Static)
        function store(state, filename)
            import edu.jiangnan.fmme.cell.sim.CellStateFixture;
        ...
        function state = load(state, filename, loadStates)
            ...
            if ~loadStates
                ...
                for i = 1:numel(names)
                    if isa(state.(names{i}), 'edu.jiangnan.fmme.cell.sim.CellState')
                        state.(names{i}) = [];
                    end
                end
            end
    ...
    methods (Static, Access = private)
        function value = determinePathPrefix()
            loc = which('edu.jiangnan.fmme.cell.sim.CellStateFixture');
