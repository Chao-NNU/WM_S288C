    ...
    methods (Static=true)
        function A = catstruct(varargin)
            ...
            for i=1:length(UFN)
                ...
                if(recurse)
                    UVAL{i} = edu.jiangnan.fmme.util.StructUtil.catstruct(VAL{idxs});
                end
            end
            VAL=UVAL;
            FN=UFN;
        ...
        function diff(struct1, struct2, indent)
            ...
            for i = 1:numel(commonFieldNames)
                ...
                switch class1                    
                    ...
                    case {'edu.jiangnan.fmme.util.SparseMat', 'edu.jiangnan.fmme.util.CircularSparseMat'}
                        subs = find(val1 ~= val2);
                    ...
                    otherwise
                        fprintf([repmat('\t', 1, indent) 'Comparing %s fields\n'], commonFieldNames{i});
                        edu.jiangnan.fmme.util.StructUtil.diff(val1, val2, indent+1);
                end
            end
        end
    end
