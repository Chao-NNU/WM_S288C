...
classdef ChromosomeCondensation < edu.jiangnan.fmme.cell.sim.Process & edu.jiangnan.fmme.cell.sim.ChromosomeProcessAspect
    
    %indices
    properties (Constant)
        optionNames__              = {}; %names of properties that are options
        fixedConstantNames__       = {
		    %'smcSepNt';
            'smcSepProbCenter';
		}; %names of fixed constant properties
    ...
    properties
        ...
        enzymeWholeCellModelIDs = {
			'YIL026C_YDL003W_YPR007C_YFL008W_YJL074C_PENTAMER';      %Chromosome Structural mataintenance ptotein cohesin
			'YIL026C_YDL003W_YPR007C_YFL008W_YJL074C_PENTAMER_ADP';  %Chromosome Structural mataintenance ptotein cohesin-ADP
			};
		enzymeIndexs_Cohesin     		= 1;	
		enzymeIndexs_Cohesin_ADP     	= 2;
		
		smcSepNt = {9210; 10840; 10210; 11090; 12020; 9310; 11980; 8930; 10230; 12030; 13340; 11740; 10500; 9930; 11980; 11700};
    end
   
    %fixed biological constants
    properties
		%smcSepNt			%1 SMC complex per len / (0.0829 * len + 5.3378) nt
    ...
    methods
        ...
        function this = ChromosomeCondensation(wholeCellModelID, name)
            this@edu.jiangnan.fmme.cell.sim.Process(wholeCellModelID, name);
        end
    end
    ...
    methods
        ...
        function [bmProd, byProd, minEnzExp, maxEnzExp] = calcResourceRequirements_LifeCycle(this, constants, states)
            ...
            nCohesins = sum(states.complexProductions(this.enzymeGlobalIndexs(this.enzymeIndexs_Cohesin_ADP)));
            ...
            byProd(this.substrateIndexs_adp)       = nATP - nCohesins;
            ...
            for i=1:16
			s(i)   = ceil(this.chromosome.sequenceLen(i) / this.smcSepNt{i});
			end
			minEnzExp(this.enzymeIndexs_Cohesin) = sum(s);
        end       
        ...
        function result = calcResourceRequirements_Current(this)
            ...
            result(this.substrateIndexs_atp)   = this.enzymes(this.enzymeIndexs_Cohesin);
            result(this.substrateIndexs_water) = this.enzymes(this.enzymeIndexs_Cohesin);
        end
        
        %simulation
        function evolveState(this)
            %% dissociate free SMC-ADP complexes
            this.enzymes(this.enzymeIndexs_Cohesin)   = this.enzymes(this.enzymeIndexs_Cohesin)   + this.enzymes(this.enzymeIndexs_Cohesin_ADP);
            this.substrates(this.substrateIndexs_adp) = this.substrates(this.substrateIndexs_adp) + this.enzymes(this.enzymeIndexs_Cohesin_ADP);
            this.enzymes(this.enzymeIndexs_Cohesin_ADP)     = 0;
            ...
            nBindingMax = min([...
                ...
                this.enzymes(this.enzymeIndexs_Cohesin)]);
            ...
            c = this.chromosome;
				[posStrnds, lens] = find(c.polymerizedRegions{16});
			
				smcPosStrands = find(c.complexBoundSites{16} == this.enzymeGlobalIndexs(this.enzymeIndexs_Cohesin_ADP));
			
				smcPosStrands = [mod(smcPosStrands(:,1) - this.smcSepNt{16}/2-this.smcSepProbCenter/2+this.enzymeDNAFootprints(this.enzymeIndexs_Cohesin_ADP)/2 -1, c.sequenceLen(16))+1  2*ceil(smcPosStrands(:,2)/2)-1;	
				mod(smcPosStrands(:,1) - this.smcSepNt{16}/2-this.smcSepProbCenter/2+this.enzymeDNAFootprints(this.enzymeIndexs_Cohesin_ADP)/2 -1, c.sequenceLen(16))+1  2*ceil(smcPosStrands(:,2)/2)];
			
				[posStrnds, lens] = c.excludeRegions(16, posStrnds, lens, smcPosStrands, this.smcSepNt{16}(ones(size(smcPosStrands, 1), 1), 1) + this.smcSepProbCenter);
			
				if isempty(posStrnds)
					return;
				end
			
				nBound = this.bindProteinToChromosomeStochastically(16,...
					this.enzymeIndexs_Cohesin_ADP,...
            ...
            this.enzymes(this.enzymeIndexs_Cohesin)         = this.enzymes(this.enzymeIndexs_Cohesin)         - nBound;
            this.enzymes(this.enzymeIndexs_Cohesin_ADP)     = this.enzymes(this.enzymeIndexs_Cohesin_ADP)     + nBound;
        ...
        function [rgnPosStrnds, rgnLens, rgnProbs] = calcNewRegions(this, i, rgnPosStrnds, rgnLens, ~, rgnIdx, offset)
            c = this.chromosome;
			[rgnPosStrnds, rgnLens] = c.excludeRegions(i, rgnPosStrnds, rgnLens, ...
                [mod(rgnPosStrnds(rgnIdx,1)+offset - this.smcSepNt{i}/2-this.smcSepProbCenter/2+this.enzymeDNAFootprints(this.enzymeIndexs_Cohesin_ADP)/2 -1, c.sequenceLen(i))+1 ...
                rgnPosStrnds(rgnIdx,2)], this.smcSepNt{i} + this.smcSepProbCenter);
            rgnProbs = max(0, rgnLens - this.enzymeDNAFootprints(this.enzymeIndexs_Cohesin_ADP) + 1);
		end
    end
end
