...
classdef ProteinFolding < edu.jiangnan.fmme.cell.sim.Process
    ...
    properties
		...
		substrateIndexs_ca		  %index within substrates of Ca2+ ion
		substrateIndexs_cu		  %index within substrates of Cu2+ ion
		...
		%}	
        enzymeWholeCellModelIDs = {     %enzyme whole cell model ids
			'YHR064C_YGR285C_DIMER';	%Ribosome-associated complex (RAC)
			'YIL142W_YJL014W_YDL143W_YJR064W_YDR188W_YJL111W_YJL008C_YDR212W_8MER';	%chaperonin-containing T-complex,TRiC/CCT
			'YFR041C_MONOMER';			%HSP40, Erj5
			'YAL005C_MONOMER';			%HSP70, Ssa1
			'YPL280W_MONOMER'};			%SMALL, Hsp32
		...
		%}
		enzymeIndexs_RAC				%index of Ribosome-associated complex (RAC) within enzymeWholeCellModelIDs
		enzymeIndexs_TRiC				%index of chaperonin-containing T-complex within enzymeWholeCellModelIDs	
		enzymeIndexs_Erj5				%index of DnaJ-like chaperone required for the folding capacity of the endoplasmic reticulum within enzymeWholeCellModelIDs
		enzymeIndexs_Ssa1				%index of Heat shock protein SSA1 within enzymeWholeCellModelIDs
		enzymeIndexs_Hsp32				%index of Probable glutathione-independent glyoxalase HSP32  within enzymeWholeCellModelIDs
    ...
    methods
        ...
        function initializeConstants(this, knowledgeBase, simulation, varargin)
            ...
            this.substrateIndexs_mg        = this.substrateIndexs({'MG2'});
            this.substrateIndexs_mn        = this.substrateIndexs({'MN2'});
            %this.substrateIndexs_sodium    = this.substrateIndexs({'NA1'});
			this.substrateIndexs_ca        = this.substrateIndexs({'CA2'});
			this.substrateIndexs_cu        = this.substrateIndexs({'CU2'});
            this.substrateIndexs_phosphate = this.substrateIndexs({'PI'});
            this.substrateIndexs_zinc      = this.substrateIndexs({'ZN2'});
			...
			%}
			this.enzymeIndexs_RAC			= this.enzymeIndexs({'YHR064C_YGR285C_DIMER'});
			this.enzymeIndexs_TRiC			= this.enzymeIndexs({'YIL142W_YJL014W_YDL143W_YJR064W_YDR188W_YJL111W_YJL008C_YDR212W_8MER'});
			this.enzymeIndexs_Erj5			= this.enzymeIndexs({'YFR041C_MONOMER'});
			this.enzymeIndexs_Ssa1			= this.enzymeIndexs({'YAL005C_MONOMER'});
			this.enzymeIndexs_Hsp32			= this.enzymeIndexs({'YPL280W_MONOMER'});
			...
			this.proteinChaperoneMatrix(1:numMonomers,this.enzymeIndexs_RAC) = 1;
			%this.proteinChaperoneMatrix(:,this.enzymeIndexs_dnaJ) = this.proteinChaperoneMatrix(:,this.enzymeIndexs_dnaK);
            this.proteinChaperoneMatrix(:,this.enzymeIndexs_Erj5) = this.proteinChaperoneMatrix(:,this.enzymeIndexs_Ssa1);
            %this.proteinChaperoneMatrix(:,this.enzymeIndexs_grpE) = this.proteinChaperoneMatrix(:,this.enzymeIndexs_Ssa1);
			this.proteinChaperoneMatrix(:,this.enzymeIndexs_TRiC) = this.proteinChaperoneMatrix(:,this.enzymeIndexs_Ssa1);
			...
			this.monomerCompartments(this.monomerCompartments == this.compartment.terminalOrganelleMembraneIndexs) = this.compartment.mitochondriaIndexs;
    ...
    methods
        ...
        function [bmProd, byProd, minEnzExp, maxEnzExp] = calcResourceRequirements_LifeCycle(this, ~, states)
			...
			%}
			minEnzExp([
			this.enzymeIndexs_RAC;
			this.enzymeIndexs_TRiC;
			this.enzymeIndexs_Erj5;
			this.enzymeIndexs_Ssa1;
			this.enzymeIndexs_Hsp32]) = 2;
