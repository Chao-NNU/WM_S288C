...
classdef RNADecay < edu.jiangnan.fmme.cell.sim.Process
    ...
    properties (Constant)
        ...
        fixedConstantNames__       = {   %names of fixed constant properties
			...
			'RibonucleaseT2likeSpecificRate';
    ...
    properties
		...
		enzymeWholeCellModelIDs = { 
			'YOL149W_YNL118C_DIMER';	  %Dcp1_Dcp2 complex
			'YPL123C_MONOMER'};           %Ribonuclease T2-like
        enzymeIndexs_Dcp1_Dcp2_complex     = 1; %index within enzymes of Dcp1_Dcp2 complex
        enzymeIndexs_Ribonuclease          = 2; %index within enzymes of Ribonuclease T2-like
    ...
    properties
		...
		RibonucleaseT2likeSpecificRate		%0.0543
    ...
    methods
        ...
        function initializeConstants(this, knowledgeBase, simulation, varargin)
            ...
            this.substrateWholeCellModelIDs = unique([simulation.state('Metabolite').wholeCellModelIDs(...
                ...
                'H';'H2O';'NH3';
                'ALA_L';'ARG_L';'ASN_L';'ASP_L';'CYS_L';'GLN_L';'GLU_L';'GLY_L';'HIS_L';'ILE_L';'LEU_L';'LYS_L';'MET_L';'PHE_L';'PRO_L';'SER_L';'THR_L';'TRP_L';'TYR_L';'VAL_L']);
            %super class method
    ...
    methods
        ...
        function [bmProd, byProd, minEnzExp, maxEnzExp] = calcResourceRequirements_LifeCycle(this, ~, states)
            ...
            aminoacylatedRNADecays = rnaDecays([this.matureTRNAIndexs]);
			...
			%}
			minEnzExp(this.enzymeIndexs_Dcp1_Dcp2_complex) = ...
                2 * (sum(rnaDecays));
            minEnzExp(this.enzymeIndexs_Ribonuclease ) = ...
                2 * sum(aminoacylatedRNADecays) / this.RibonucleaseT2likeSpecificRate;
            maxEnzExp = Inf(size(this.enzymeWholeCellModelIDs));
        end
        ...
        function result = calcResourceRequirements_Current(this)
			...
			if this.enzymes(this.enzymeIndexs_Dcp1_Dcp2_complex) == 0
            ...
            for i = 1:16
				if ~isempty(this.transcripts.abortedTranscripts{i})
					result(this.substrateIndexs_water) = ...
						+ result(this.substrateIndexs_water) ...
						+ sum(this.transcripts.abortedTranscripts{i}(:, 2) - 1);
				end
			end
        end
        ...
        function evolveState(this)
			...
			Dcp1_Dcp2_complex = this.enzymes(this.enzymeIndexs_Dcp1_Dcp2_complex);
            %{
			peptidylTRNAHydrolase = this.randStream.stochasticRound(...
                ...
                * this.stepSizeSec);
            %}
			Ribonuclease = this.randStream.stochasticRound(...
				this.enzymes(this.enzymeIndexs_Ribonuclease) ...
				* this.RibonucleaseT2likeSpecificRate ...
                * this.stepSizeSec);	
				
            %Ribonuclease R required for decay, terminate early if no ribonuclease R
            %if ribonucleaseR == 0
			if Dcp1_Dcp2_complex == 0
                return;
            end
            
            %% decay all aborted transcripts
			for j = 1:16
            abortedSeqs{j} = this.transcripts.abortedSequences{j};
            abortedTfs{j} = false(size(abortedSeqs{j}));
            for i = 1:numel(abortedSeqs{j})
                substrateCost{j} = ssRNA.computeDecayReaction(ssRNA.computeBaseCount(...
                    abortedSeqs{j}{i}, numel(this.substrates), this.substrateIndexs_nmps), ...
                    numel(abortedSeqs{j}{i}), 'linear', ...
            ...
            this.transcripts.abortedTranscripts{j} = this.transcripts.abortedTranscripts{j}(~abortedTfs{j}, :);
        ...
        function value = getDryWeight(this)
            if size(this.RNAs, 3) == 1
                value = this.getDryWeight@edu.jiangnan.fmme.cell.sim.Process() + ...
                    this.rna.molecularWeights' * this.RNAs / edu.jiangnan.fmme.util.ConstantUtil.nAvogadro;
            else
                value = this.getDryWeight@edu.jiangnan.fmme.cell.sim.Process() + ...
                    permute(this.rna.molecularWeights' * permute(this.RNAs,[1 3 2]),[1 3 2]) / edu.jiangnan.fmme.util.ConstantUtil.nAvogadro;
            end
        end
    end
end
