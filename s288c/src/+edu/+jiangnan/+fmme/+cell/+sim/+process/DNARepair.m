...
%  12. Witte G, Hartung S, B?ttner K, Hopfner KP (2008). Structural.
...
classdef  DNARepair < edu.jiangnan.fmme.cell.sim.ReactionProcess & edu.jiangnan.fmme.cell.sim.ChromosomeProcessAspect
    ...
    properties
        ...
        substrateIndexs_FADH2                       %index within substrates of FADH2
    ...
    properties        
        ...
        RM_MunI_RecognitionSequence				    %MunI recognition sequence                       CAATTG            [PUB_0096]
    ...
    methods
        ...
        function initializeConstants(this, knowledgeBase, simulation, varargin)
            ...
            %% additional enzymes
            enzymeWholeCellModelIDs = unique([this.enzymeWholeCellModelIDs; 'YPL194W_YLR288C_YOR368W_TRIMER']);
			...
			this.HR_RecU_CleavageSequence     = 'WTTS';
			this.RM_EcoD_RecognitionSequence  = 'TTANNNNNNNGTCY';
			this.RM_MunI_RecognitionSequence  = 'CAATTG';
            ...
            this.substrateIndexs_FADH2                = this.substrateIndexs({'FADH2'});
			this.substrateIndexs_AMP                  = this.substrateIndexs({'AMP'});
            %this.substrateIndexs_MLTHF                = this.substrateIndexs({'MLTHF'});
            ...
            this.substrateIndexs_undamagedNucleobases = this.substrateIndexs({'ADE';'CSN';'GUA';'THY'});
            ...
            %% enzyme indices
            this.enzymeIndexs_BER_baseExcision               = this.enzymeIndexs({'YKL114C_MONOMER'; 'YBL019W_MONOMER'});
            this.enzymeIndexs_formamidopyrimidineGlycosylase = this.enzymeIndexs({'YML060W_MONOMER'});         %BER  fpg          formamidopyrimidine-DNA glycosylase
            this.enzymeIndexs_uracilGlycosylase              = this.enzymeIndexs({'YML021C_MONOMER'});         %BER  ung          uracil-DNA glycosylase, putative
            this.enzymeIndexs_apurinicEndonuclease           = this.enzymeIndexs({'YAL015C_MONOMER'; 'YOL043C_MONOMER'});  %BER  nfo  apurinic endonuclease
            this.enzymeIndexs_incisionComplex                = this.enzymeIndexs({'YPL022W_YML095C_YMR201C_TRIMER'}); %NER  uvrABC       DNA incision complex
			this.enzymeIndexs_helicase35                     = this.enzymeIndexs({'YER171W_MONOMER'});         %NER  pcrA         3'-5' helicase
			this.enzymeIndexs_recombinationStrandExchange    = this.enzymeIndexs({'YER179W_MONOMER'});         %HR   recA/DMC1    recombination protein, strand exchange
            this.enzymeIndexs_hollidayJunctionEndonuclease   = this.enzymeIndexs({'YER041W_MONOMER'});         %HR   recU/YEN1    Holliday junction endonuclease
            this.enzymeIndexs_hollidayJunctionHelicase       = this.enzymeIndexs({'YBR073W_MONOMER'});         %HR   RDH54        Holliday junction DNA helicase
            this.enzymeIndexs_exonuclease53                  = this.enzymeIndexs({'YDR369C_MONOMER'});         %HR   pol I-like   5'-3' exonuclease, putative
            this.enzymeIndexs_polymerase                     = this.enzymeIndexs({'YCR014C_YDL147W_DIMER'});   %dnaN DNA polymerase III, beta subunit
            this.enzymeIndexs_ligase                         = this.enzymeIndexs({'YDL164C_MONOMER';'YOR005C_MONOMER'}); %ligA DNA ligase, NAD-dependent
            this.enzymeIndexs_phosphoesterase                = this.enzymeIndexs({'YLR059C_MONOMER'});         %mgpA phosphoesterase
            this.enzymeIndexs_DisA                           = this.enzymeIndexs({'YPL194W_YLR288C_YOR368W_TRIMER'});  %disA      DNA integrity scanning protein
            this.enzymeIndexs_RM_typeI                       = this.enzymeIndexs({'YIL153W_MONOMER'});         %type I restriction modification DNA specificity domain protein
            this.enzymeIndexs_RM_typeII                      = this.enzymeIndexs({'YKL025C_MONOMER'});         %adenine-specific DNA modification methylase
            ...
            %base excision repair (BER)
            this.reactionIndexs_BER                = find(strcmp(this.reactionTypes,'base_excision_repair') | strcmp(this.reactionTypes,'base_excision_repair_ base_excision'));
            this.reactionIndexs_BER_baseexcision   = find(strcmp(this.reactionTypes,'base_excision repair_base_excision'));
            this.reactionIndexs_BER_aplyase        = this.reactionIndexs({'AP_lyase_1'; 'AP_lyase_2'});
            this.reactionIndexs_BER_drpesterase    = this.reactionIndexs({'deoxyribosephosphodiesterase'});            
            this.reactionIndexs_BER_apendonuclease = this.reactionIndexs({'AP_endonuclease_1'; 'AP_endonuclease_2'});
            ...
            this.reactionIndexs_NER                   = find(strcmp(this.reactionTypes,'nucleotide_excision_repair'));
            ...
            %cleavage
            this.reactionIndexs_DNACleavage = find(strcmp(this.reactionTypes,'DNA_cleavage'));
            ...
            %homologous recombination (HR) double strand break repair (DSBR)
            this.reactionIndexs_HR_dsbr               = find(strcmp(this.reactionTypes,'homologous_recombination'));
            ...
            for i = 1:16
            this.hollidayJunctionResolutionSites{i} = this.chromosome.sequence{i}.findSubsequence(...
                this.HR_RecU_CleavageSequence, this.HR_RecU_CleavagePosition, true);
			end
            %polymerization
            this.reactionIndexs_polymerization = find(strcmp(this.reactionTypes,'DNA_polymerization'));
            ...
            %ligation
            this.reactionIndexs_ligation = this.reactionIndexs({'DNA_ligation_repair_1';'DNA_ligation_repair_2'});
			...
			for i = 1:16
            this.RM_EcoD_RecognitionSites{i} = this.chromosome.sequence{i}.findSubsequence(this.RM_EcoD_RecognitionSequence, 1);
            this.RM_EcoD_RecognitionSites{i} = mod(...
                repmat(this.RM_EcoD_RecognitionSites{i},1,length(this.RM_EcoD_RecognitionSequence)) + ...
                repmat(0:length(this.RM_EcoD_RecognitionSequence)-1,size(this.RM_EcoD_RecognitionSites{i},1),1) - ...
                1, this.chromosome.sequenceLen(i))+1;

            this.RM_MunI_RecognitionSites{i} = this.chromosome.sequence{i}.findSubsequence(this.RM_MunI_RecognitionSequence, 1);
            
			this.RM_MunI_RecognitionSites{i} = mod(...
                repmat(this.RM_MunI_RecognitionSites{i},1,length(this.RM_MunI_RecognitionSequence)) + ...
                repmat(0:length(this.RM_MunI_RecognitionSequence)-1,size(this.RM_MunI_RecognitionSites{i},1),1) - ...
                1, this.chromosome.sequenceLen(i))+1;
			end
            % reactionSmallMoleculeStoichiometryMatrix
    ...
    methods
        ...
        function [bmProd, byProd, minEnzExp, maxEnzExp] = calcResourceRequirements_LifeCycle(this, constants, ~)
            ...
            %enzymes
            minEnzExp = max(...
                this.reactionCatalysisMatrix' * (rates ./ this.enzymeBounds(:, 2)) / constants.states.Time.cellCycleLength * log(1.111), ...
                this.reactionCatalysisMatrix(this.reactionIndexs_RM_MunI_Methylation, :)' * ...
                    (rates(this.reactionIndexs_RM_MunI_Methylation) ./ this.enzymeBounds(this.reactionIndexs_RM_MunI_Methylation, 2)) / constants.states.Time.cellCycleLength * log(1.111) / ...
                    (exp(log(1.111) * (constants.states.Time.replicationInitiationDuration + constants.states.Time.replicationDuration) / constants.states.Time.cellCycleLength) - ...
                     exp(log(1.111) * constants.states.Time.replicationInitiationDuration / constants.states.Time.cellCycleLength)) ...
                );
        ...
        function repairRates = calcExpectedReactionRates(this, constants)
			...
			for j = 1:16
			d{j} = dmg.reactionVulnerableMotifs;
			end
			for j = 1:16
            polDNA{j} = collapse(c.polymerizedRegions{j});
            bases = 'ACGT';
            dntpComposition{j} = [
                1 - c.sequenceGCContent(j)
                c.sequenceGCContent(j)
                c.sequenceGCContent(j)
                1 - c.sequenceGCContent(j)] / 2;
            ...
            for i = 1:size(dmg.reactionStoichiometryMatrix, 2)
				...
				if ischar(d{j}{i})
                    %nVulnerableSites{j} = polDNA{j} * prod(dntpComposition{j}(ismembc(dmg.reactionVulnerableMotifs{i}, bases)));
					nVulnerableSites{j} = polDNA{j} * prod(dntpComposition{j}(ismembc(d{j}{i}, bases)));
				else
                    %nVulnerableSites{j} = collapse(...
                        %dmg.reactionVulnerableMotifs{i} == c.(dmg.reactionVulnerableMotifTypes{i})(1) & ...
                        %dmg.reactionVulnerableMotifs{i} == c.damagedSites_nonRedundant{1});
					nVulnerableSites{j} = collapse(...	
						d{j}{i} == c.(dmg.reactionVulnerableMotifTypes{i}){j} & ...
						d{j}{i} == c.damagedSites_nonRedundant{j});					
                end
                if nVulnerableSites{j} == 0
                    continue;
                end
                ...
                damageRate{j} = nVulnerableSites{j} * ...
                ...
                if ~isempty(iRadLcl)
                    ...
                    damageRate{j} = damageRate{j} * this.stimulus.values(iRadGbl);
                end
                ...
                switch dmg.reactionDamageTypes{i}
                    case 'abasicSites'
                        ...
                        repairRates(this.reactionIndexs_polymerization) = ...
                            repairRates(this.reactionIndexs_polymerization) + ...
                            damageRate{j} * dntpComposition{j};
                    ...
                    case 'intrastrandCrossLinks'
                        ...
                        repairRates(this.reactionIndexs_NER_excision) = ...
                            repairRates(this.reactionIndexs_NER_excision) + ...
                            damageRate{j} * ceil(numExcisions / this.NER_PcrA_StepSize);
                        repairRates(this.reactionIndexs_DNACleavage) = ...
                            repairRates(this.reactionIndexs_DNACleavage) + ...
                            damageRate{j} * numExcisions * dntpComposition{j};
                        
                        repairRates(this.reactionIndexs_polymerization) = ...
                            repairRates(this.reactionIndexs_polymerization) + ...
                            damageRate{j} * 2 * dntpComposition{j};
                        repairRates(this.reactionIndexs_ligation) = ...
                            repairRates(this.reactionIndexs_ligation) + ...
                            damageRate{j};
                    case 'strandBreaks'
            ...
            repairRates(this.reactionIndexs_RM_MunI_Methylation) = 2 * size(this.RM_MunI_RecognitionSites{j}, 1);
			end
        end
        ...
        function initializeState(this)
            ...
            c.damagedBases{i}(this.RM_MunI_RecognitionSites{i}(:, this.RM_MunI_MethylatedPositions(1)), 1) = iMet;
            c.damagedBases{i}(this.RM_MunI_RecognitionSites{i}(:, this.RM_MunI_MethylatedPositions(2)), 2) = iMet;
			end
		end
        ...
        function result = calcResourceRequirements_Current(this)
            ...
            for i = 1:16
            %% numbers of damages
            nGapSites{i} = nnz(c.gapSites{i});
            nAbasicSites{i} = nnz(c.abasicSites{i});
            if nnz(c.damagedSites{i})
                [posStrnds{i}, bases{i}] = find(c.damagedBases{i});
                nDamagedBases{i} = numel(bases{i}) - sum(...
                    bases{i} == this.metabolite.m6ADIndexs & ...
                    ((isodd(posStrnds{i}(:, 2)) & ismembc(posStrnds{i}(:, 1), this.RM_MunI_RecognitionSites{i}(:, this.RM_MunI_MethylatedPositions(1)))) | ...
                    (iseven(posStrnds{i}(:, 2)) & ismembc(posStrnds{i}(:, 1), this.RM_MunI_RecognitionSites{i}(:, this.RM_MunI_MethylatedPositions(2))))));
            else
                nDamagedBases{i} = 0;
            end
            nIntrastrandCrossLinks{i} = nnz(c.intrastrandCrossLinks{i});
            nStrandBreaks{i} = nnz(c.strandBreaks{i});
            if nStrandBreaks{i} > 0
                nDSBR{i} = nnz(c.doubleStrandBreaks{i});
            else
                nDSBR{i} = 0;
            end
            nHJ{i} = nnz(c.hollidayJunctions{i}) / 2;
            
            %% reaction rates
            rates = zeros(size(this.reactionSmallMoleculeStoichiometryMatrix, 2), 1);
            
            %BER
            rates(this.reactionIndexs_BER_baseexcision)   = nDamagedBases{i};
            rates(this.reactionIndexs_BER_apendonuclease) = 0.5 * nAbasicSites{i};
            rates(this.reactionIndexs_BER_aplyase)        = 0.5 * nAbasicSites{i};
            rates(this.reactionIndexs_BER_drpesterase)    = 0.5 * nAbasicSites{i};
            rates(this.reactionIndexs_BER_dr5plyase)      = 0.5 * nAbasicSites{i};
            
            %NER
            nNER{i} = nDamagedBases{i} + nIntrastrandCrossLinks{i};
            nExcisions = (this.NER_UvrABC_IncisionMargin5 + this.NER_UvrABC_IncisionMargin3 + 1);
            rates(this.reactionIndexs_NER_incision) = nNER{i};
            rates(this.reactionIndexs_NER_excision) = nNER{i} * nExcisions;
            rates(this.reactionIndexs_DNACleavage)  = nNER{i} * nExcisions/4;
            
            %HR
            numResections = 2 * this.HR_PolA_ResectionLength;
            avgJunctionMigrationLength{i} = c.sequenceLen(i) / (size(this.hollidayJunctionResolutionSites{i}, 2) / 2) / 2;
            rates(this.reactionIndexs_HR_DNAResection)       = nDSBR{i} * numResections;
            rates(this.reactionIndexs_HR_strandExchange)     = nDSBR{i} * 2 + nHJ{i} * 2;
            rates(this.reactionIndexs_HR_junctionMigration)  = nHJ{i} * avgJunctionMigrationLength{i} / this.HR_RuvAB_JunctionMigrationHop;
            rates(this.reactionIndexs_HR_junctionResolution) = nHJ{i};
            
            %polymerization/ligation
            if nGapSites{i} && nAbasicSites{i}
                rates(this.reactionIndexs_polymerization) = nnz(c.gapSites{i} & c.abasicSites{i});
            end
            rates(this.reactionIndexs_ligation) = nStrandBreaks{i};
            
            %restriction / modification
            rates(this.reactionIndexs_RM_MunI_Methylation) = nnz(c.hemiunmethylatedMunIRMSites{i});
            rates(this.reactionIndexs_RM_MunI_Restriction) = nnz(c.restrictableMunIRMSites{i});
        ...
        function evolveState(this)
			for i = 1:16
            if nnz(this.chromosome.damagedSites{i}) == 0
                %there is no damage, no repair necessary
                if this.randStream.rand > 0.5
                    this.evolveState_Modification(i);
                    this.evolveState_Restriction(i);
                else
                    this.evolveState_Restriction(i);
                    this.evolveState_Modification(i);
                end
    ...
    methods
        ...
        function evolveState_BER(this)
            ...
            if nnz(c.damagedBases{i})
                ...
                for idx = 1:length(this.enzymeIndexs_BER_baseExcision)
                    j = randOrder(idx);
                    ezIdx = this.enzymeIndexs_BER_baseExcision(j);
                    ...
                    [positionsStrands, damagedBaseGlobalIndexs] = find(c.damagedBases{i});
                    ...
                    c.damagedBases{i}(positionsStrands) = 0;
                    c.abasicSites{i}( positionsStrands) = 1;
                end
            end
            ...
            if nnz(c.abasicSites{i})
                ...
                for j = 1:length(rxIdxs)
                    rxIdx = rxIdxs(randOrder(j));
                    ezIdx = find(this.reactionCatalysisMatrix(rxIdx,:));
                    if this.enzymes(ezIdx) == 0; continue; end
                    
                    positionsStrands = find(c.abasicSites{i} & xor(c.abasicSites{i},  c.strandBreaks3{i} | c.strandBreaks5{i}));
                    if isempty(positionsStrands); continue; end
                    ...
                    if rxIdx == this.reactionIndexs_BER_aplyase
                        ...
                        c.strandBreaks{i}(c.shiftPositionsStrandsBond3Prime(positionsStrands)) = 1;
                        c.damagedSugarPhosphates{i}(positionsStrands) = 0;
                    else
                        checkPositionsStrands = positionsStrands;
                        ...
                        c.strandBreaks{i}(c.shiftPositionsStrandsBond5Prime(positionsStrands)) = 1;
                        c.damagedSugarPhosphates{i}(positionsStrands) = 0;
                    end
            ...
            if nnz(c.abasicSites{i}) && nnz(c.strandBreaks{i})
                ...
                if ~isempty(positionsStrands)
                    ...
                    if numReactions > 0
                        ...
                        c.damagedSugarPhosphates{i}(positionsStrands) = 0;
                        c.strandBreaks{i}(c.shiftPositionsStrandsBond5Prime(positionsStrands)) = 1;
                        c.gapSites{i}(positionsStrands) = 1;
            ...
            if nnz(c.abasicSites{i}) && nnz(c.strandBreaks{i})
                positionsStrands = find(c.abasicSites{i} & c.strandBreaks5{i});
                if ~isempty(positionsStrands)
        ...
        function evolveState_NER(this)            
            ...
            %no damage
            if nnz(c.damagedBases{i}) == 0 && nnz(c.intrastrandCrossLinks{i}) == 0
                return;
            end

            %% damaged sites: positions and strands
            damagedPositionsStrands = setdiff(...
                find(xor(c.damagedBases{i}, c.intrastrandCrossLinks{i})), ...
                find(c.damagedBases{i} == this.substrateGlobalIndexs(this.substrateIndexs_m6AD)), ...
                'rows');
            ...
            rmPositionsStrands(...
                c.damagedBases{i}(rmPositionsStrands) == this.substrateGlobalIndexs(this.substrateIndexs_m6AD), :) = [];
            
            damagedPositionsStrands(edu.jiangnan.fmme.util.SparseMat.ismember_subs(damagedPositionsStrands, rmPositionsStrands, [c.sequenceLen(i) c.nCompartments]), :) = [];
            ...
            idxs = find(c.intrastrandCrossLinks{i}(damagedPositionsStrands));
            ...
            rxCounts(3:6) = c.sequence{i}.subsequenceBaseCounts(...
            ...
            damages = c.damagedBases{i}(damagedPositionsStrands);
            idxs = find(damages);
            undamagedBaseCounts = c.sequence{i}.subsequenceBaseCounts(damagedPositionsStrands(idxs, :));
            [damages, damageCounts] = edu.jiangnan.fmme.util.countUnique(damages(idxs, :));
            ...
            damages = c.intrastrandCrossLinks{i}(damagedPositionsStrands);
            idxs = find(damages);
            undamagedBaseCounts = c.sequence{i}.subsequenceBaseCounts(...
            ...
            c.strandBreaks{i}(          incisedPositionsStrands) = 1;
            c.gapSites{i}(              excisedPositionsStrands) = 1;
            c.abasicSites{i}(           excisedPositionsStrands) = 1;
            c.damagedBases{i}(          excisedPositionsStrands) = 0;
            c.intrastrandCrossLinks{i}( excisedPositionsStrands) = 0;
			end
		end
        ...
        function evolveState_HR(this)
            ...
            if nnz(c.strandBreaks{i}) && nnz(c.doubleStrandBreaks{i})
                %find double strand breaks
                positionsChromosomes = find(c.doubleStrandBreaks{i}(:, 1:2:end));
                ...
                if numReactions > 0
                    ...
                    c.gapSites{i}(   reshape(resectedPositions1, [], 1), 1) = 1;
                    c.gapSites{i}(   reshape(resectedPositions2, [], 1), 2) = 1;
                    c.abasicSites{i}(reshape(resectedPositions1, [], 1), 1) = 1;
                    c.abasicSites{i}(reshape(resectedPositions2, [], 1), 2) = 1;
                    c.hollidayJunctions{i}(  resectedEndPositions1,     1) = 1;
                    c.hollidayJunctions{i}(  resectedEndPositions2,     2) = 1;
                    
                    rxIdxs = [this.reactionIndexs_HR_strandExchange; this.reactionIndexs_HR_DNAResection];
                    rxCounts = [2*numReactions;
                        c.sequence{i}.subsequenceBaseCounts(resectedPositions1, 1) + c.sequence{i}.subsequenceBaseCounts(resectedPositions2, 2)];
                    this.substrates = this.substrates + ...
                        this.reactionSmallMoleculeStoichiometryMatrix(:, rxIdxs) * rxCounts;
            ...
            if nnz(c.hollidayJunctions{i})
                %find holiday junctions
                numResections = 2*this.HR_PolA_ResectionLength;
                positionsChromosomes = find(...
                    c.hollidayJunctions{i}(:, 1:2:end) & ...
                    vertcat(c.hollidayJunctions{i}(end-numResections+1:end,2:2:end), c.hollidayJunctions{i}(1:end-numResections, 2:2:end)));
                ...
                for j = 1:numel(positions)%change from i to j
                    %formation site
                    hollidayJunctionFormationSite = mod(positions(j) + [0; -numResections] - 1, c.sequenceLen(i)) + 1;
                    
                    %find nearest resolution site
                    hollidayJunctionResolutionSite = zeros(2,1);
                    if hollidayJunctionFormationSite(1) > this.hollidayJunctionResolutionSites{i}(find(this.hollidayJunctionResolutionSites{i}(:, 2) == 1, 1, 'last'), 1)
                        hollidayJunctionResolutionSite(1) = this.hollidayJunctionResolutionSites{i}(...
                            find(this.hollidayJunctionResolutionSites{i}(:, 2) == 1, 1, 'first'), 1);
                    else
                        hollidayJunctionResolutionSite(1) = this.hollidayJunctionResolutionSites{i}(...
                            find(this.hollidayJunctionResolutionSites{i}(:, 2) == 1 & ...
                            this.hollidayJunctionResolutionSites{i}(:, 1) > hollidayJunctionFormationSite(1), 1, 'first'), 1);
                    end
                    if hollidayJunctionFormationSite(2) < this.hollidayJunctionResolutionSites{i}(find(this.hollidayJunctionResolutionSites{i}(:, 2) == 2, 1, 'first'), 1)
                        hollidayJunctionResolutionSite(2) = this.hollidayJunctionResolutionSites{i}(...
                            find(this.hollidayJunctionResolutionSites{i}(:, 2) == 2, 1, 'last'), 1);
                    else
                        hollidayJunctionResolutionSite(2) = this.hollidayJunctionResolutionSites{i}(...
                            find(this.hollidayJunctionResolutionSites{i}(:, 2) == 2 & ...
                            this.hollidayJunctionResolutionSites{i}(:, 1) < hollidayJunctionFormationSite(2), 1, 'last'), 1);
                    end
                    ...
                    resolutionRegionLength = mod(hollidayJunctionResolutionSite(1) - hollidayJunctionResolutionSite(2) + 1 -1, c.sequenceLen(i)) + 1;
                    ignoreDamageFilter = mod(repmat(hollidayJunctionFormationSite, 1, 3) + repmat(-1:1, 2, 1) - hollidayJunctionResolutionSite(2) + 1 -1, c.sequenceLen(i)) + 1;
                    if ~this.bindProteinToChromosome([hollidayJunctionResolutionSite(2) 2*chrs(j)], ...
                    ...
                    c.hollidayJunctions{i}([hollidayJunctionFormationSite 2*chrs(j) + [-1;0]]) = 0;
                    c.strandBreaks{i}([hollidayJunctionResolutionSite 2*chrs(j) + [-1;0]]) = 1;
					end
				end
            end
        ...
        function evolveState_Polymerize(this)
            c = this.chromosome;
            for i = 1:16
            if nnz(c.gapSites{i}) == 0 || nnz(c.abasicSites{i}) == 0 || nnz(c.strandBreaks{i}) == 0
                return;
            end
            
            %find regions that need to be polymerized
            gapStarts = find(c.gapSites{i} & c.abasicSites{i} & c.strandBreaks5{i});
            if isempty(gapStarts)
                return; 
            end
            gapNotStarts = find(c.gapSites{i} & c.abasicSites{i} & c.gapSites5{i} & c.abasicSites5{i});
            gapNotStarts(find(c.strandBreaks5{i}(gapNotStarts)), :) = []; %#ok<FNDSB>
            gaps = edu.jiangnan.fmme.util.SparseMat.sort_subs([gapStarts;gapNotStarts], [c.sequenceLen(i) c.nCompartments]);
            gaps(...
                c.damagedSugarPhosphates3{i}(gaps) | ...
                c.damagedSugarPhosphates5{i}(gaps) | ...
                c.damagedBases{i}(           gaps) | ...
                c.intrastrandCrossLinks3{i}( gaps) | ...
                c.intrastrandCrossLinks5{i}( gaps), :)=[];
            
            [positions, lengths, strands, sequencesCell] = c.sequence{i}.locateFeatures(gaps);
            ...
            for j = 1:numel(positions)
                ...
                polymerizedBases = edu.jiangnan.fmme.cell.sim.util.polymerize(...
                    sequencesCell{k}(1: min(lengths(k), polymerase)), ...
                ...
                ignoreDamageFilter = dirs(k) * (ignoreDamageMin:ignoreDamageMax) + 1;
                
                %limit extent by accessibility of sites to DNA polymerase
                [~, ~, ~, ~, polymerizedBases] = this.bindProteinToChromosome(c.shiftPositionsStrandsBond5Prime([positions(k) strands(k)]), this.enzymeIndexs_polymerase, 1, [], ...
                    false, true, dirs(k) * (polymerizedBases + 1), true, ignoreDamageFilter);
                
                polymerizedBases = min(abs(polymerizedBases), lengths(k));
                ...
                [~, ~, baseCosts] = edu.jiangnan.fmme.cell.sim.util.polymerize(...
                    sequencesCell{k}(1:polymerizedBases), ...
                    this.substrates(this.substrateIndexs_dNTPs), 'ACGT', char(0), 0, 0, this.randStream);
                ...
                pos = positions(k) + dirs(k) * (0:polymerizedBases-1);
                c.gapSites{i}(   pos, strands(k)) = 0;
                c.abasicSites{i}(pos, strands(k)) = 0;
                c.strandBreaks{i}(c.shiftPositionsStrandsBond5Prime([pos(1)   strands(k)])) = 0;
                c.strandBreaks{i}(c.shiftPositionsStrandsBond3Prime([pos(end) strands(k)])) = 1;
				end
			end
        end
        ...
        function evolveState_Ligate(this, i)
            c = this.chromosome;
			%for i = 1:16
            if nnz(c.strandBreaks{i}) == 0
                return;
            end
            
            positionsStrands{i} = find(c.singleStrandBreaks{i});
            if isempty(positionsStrands{i})
                return;
            end
            
            numReactions{i} = max(0, floor(min([
                size(positionsStrands{i}, 1);
            ...
            [~, ~, positionsStrands{i}, numReactions{i}] = this.bindProteinToChromosome(i, ...
                positionsStrands{i}, this.enzymeIndexs_ligase, numReactions{i}, [], false, true, 1, false, [0 1 2]);            
            if numReactions{i} == 0
                return;
            end
            
            c.strandBreaks{i}(positionsStrands{i}) = 0;
            
            this.substrates = this.substrates + numReactions{i} * ...
        ...
        function evolveState_DisA(this, i)
            %find locations of damages
			%for i = 1:16
            positionsStrands{i} = find(this.chromosome.damagedSites_excm6AD{i});
            if isempty(positionsStrands{i})
                return;
            end
            
            %bind free DisA to accessible damaged sites
            this.bindProteinToChromosome(i, positionsStrands{i}, this.enzymeIndexs_DisA, ...
                this.enzymes(this.enzymeIndexs_DisA), [], true, true, 1, false, 1);
			%end
		end
        
        function evolveState_Modification(this, i)
            c = this.chromosome;
            %for i = 1:16
            ...
            posStrnds{i} = find(c.hemiunmethylatedMunIRMSites{i});
            ...
            [~, ~, posStrnds{i}, numReactions{i}] = this.bindProteinToChromosome(i, ...
                posStrnds{i}, this.enzymeIndexs_RM_typeII, numReactions{i}, [], false, true, 1, false, [0 1 2]);
            if numReactions{i} == 0
                return;
            end
            
            c.damagedBases{i}(posStrnds{i}) = this.substrateGlobalIndexs(this.substrateIndexs_m6AD);
            
            this.substrates = this.substrates + ...
                numReactions{i} * this.reactionSmallMoleculeStoichiometryMatrix(:, this.reactionIndexs_RM_MunI_Methylation);
			%end
		end
        
        function evolveState_Restriction(this, i)
            c = this.chromosome;
            %for i = 1:16
            %% EcoD Type I restriction/modification system
            %  M. genitalium contains only the DNA binding subunit
            ...
            posStrnds{i} = find(c.restrictableMunIRMSites{i});
            ...
            [~, ~, posStrnds{i}, numReactions{i}] = this.bindProteinToChromosome(i, ...
                posStrnds{i}, this.enzymeIndexs_RM_typeII, numReactions{i}, [], false, true, 1, false, -1:1);
            if numReactions{i} == 0
                return;
            end
            
            c.strandBreaks{i}(posStrnds{i}) = 1;
            
            this.substrates = this.substrates + ...
                numReactions{i} * this.reactionSmallMoleculeStoichiometryMatrix(:, this.reactionIndexs_RM_MunI_Restriction);
			%end
		end
    end
end