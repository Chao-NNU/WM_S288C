...
for i = 1:numel(varargin)
    if isa(varargin{i}, 'edu.jiangnan.fmme.cell.sim.util.Logger') || ...
            (iscell(varargin{i}) && isa(varargin{i}{1}, 'edu.jiangnan.fmme.cell.sim.util.Logger'))
        loggers = varargin{i};
        if ~iscell(loggers)
            loggers = {loggers};
        end
...
try    
    for i = 1:this.getNumSteps
        ...
        met.processRequirements = edu.jiangnan.fmme.util.SparseMat(requirements);
        met.processAllocations = edu.jiangnan.fmme.util.SparseMat(allocations);
        met.processUsages = edu.jiangnan.fmme.util.SparseMat(usages);
