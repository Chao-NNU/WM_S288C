...
classdef Population
    methods (Static = true)
        function run(simBatchDir, fileName, selectedSimulations, methodsToRun)
            ...
            %pdf/xls files
            metaData = meta.class.fromName('edu.jiangnan.fmme.cell.sim.analysis.Population');
        ...
        function figData = growth(figHandle, simBatchDir, selectedSimulations)
            import edu.jiangnan.fmme.cell.sim.util.MacromoleculeUtil;
            import edu.jiangnan.fmme.cell.sim.util.SimulationEnsemble;
            import edu.jiangnan.fmme.cell.sim.util.SimulationDiskUtil;
            import edu.jiangnan.fmme.cell.sim.util.PlotUtil;
            import edu.jiangnan.fmme.cell.sim.analysis.CellOverview;
        ...
        function figData = macromoleculeHalfLives(figHandle, simBatchDir, selectedSimulations)
            ...
            for i = 1:nSims
                ...
                if isa(states.Rna.counts, 'edu.jiangnan.fmme.util.SparseMat')
                    ...
                    tmp = edu.jiangnan.fmme.util.SparseMat(subs, vals, [6 * numel(rna.matureIndexs) 1 size(states.Rna.counts, 3)]);
            ...
            for i = 1:nSims
                ...
                for j = 1:numel(startRibs)
                    ...
                    if states.Ribosome.mRNAPositions(startRibs(j), startTimes(j)) == 0
                        ...
                        aaIdx = find(aa == edu.jiangnan.fmme.cell.kb.ProteinMonomer.bases);
                    end
    ...
    methods (Static = true)
        function [unsyncTime, unsyncData, cellCount, simOrder] = desynchronizePopulation(sim, syncTime, syncData, simEndTimes)
            ...
            randStream = edu.jiangnan.fmme.util.RandStream('mcg16807');
