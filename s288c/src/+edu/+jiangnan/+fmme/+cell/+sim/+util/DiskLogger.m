...
classdef DiskLogger < edu.jiangnan.fmme.cell.sim.util.Logger
    %options
    ...
    methods (Static)
        ...
        function states = downSample_extract_byState(states, options, metadata, outputDirectory, stateNames, initTime, finTime, downsampleStepSec)
            ...
            for i = 1:numel(sNames);
                ...
                for j = 1:size(propNames, 1)
                    ...
                    if  isa(tmp, 'edu.jiangnan.fmme.util.SparseMat') && ...
                            ~isa(states.(stateName).(propNames{j, 1}), 'edu.jiangnan.fmme.util.SparseMat')
                        tmp = full(tmp);
                    end
            ...
            for i = 1:numSegments
                ...
                for j = 1:size(stateNames, 1)
                    try
                        ...
                        if isa(states.(stateNames{j, 1}).(stateNames{j, 2}), 'edu.jiangnan.fmme.util.SparseMat')
                            if ~isa(tmp, 'edu.jiangnan.fmme.util.SparseMat')
                                tmp = edu.jiangnan.fmme.util.SparseMat(tmp);
                            end
                            tmp2{i, j} = tmp;
                        else
                ...
                if isa(states.(stateNames{j, 1}).(stateNames{j, 2}), 'edu.jiangnan.fmme.util.SparseMat')
                    ...
                    for i = 1:numel(tmp)
                        if size(tmp{i}, 1) ~= siz(1)
                            tmp{i} = [tmp{i}; edu.jiangnan.fmme.util.SparseMat([], [], [siz(1)-size(tmp{i}, 1) size(tmp{i}, 2) size(tmp{i}, 3)])];
                        end
                        if size(tmp{i}, 2) ~= siz(2)
                            tmp{i} = [tmp{i}  edu.jiangnan.fmme.util.SparseMat([], [], [size(tmp{i}, 1) siz(2)-size(tmp{i}, 2) size(tmp{i}, 3)])];
                        end
                    end
        ...
        function data = allocateData(dataType, sz, sparsity)
            ...
            if prod(sz) > 10^8 && (nargin < 3 || sparsity < 1/3) && ...
                ...
                dataType = 'edu.jiangnan.fmme.util.SparseMat';
            end
