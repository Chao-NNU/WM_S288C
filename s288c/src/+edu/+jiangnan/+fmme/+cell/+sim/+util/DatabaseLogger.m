...
classdef DatabaseLogger < edu.jiangnan.fmme.cell.sim.util.Logger
    %options
    ...
    methods
        ...
        function this = finalize(this, sim)
            ...
            contactWID = edu.jiangnan.fmme.cell.kb.KnowledgeBaseUtil.getContact(...
    ...
    methods
        ...
        function copyFromState(this, sim, k)
            import edu.jiangnan.fmme.cell.sim.util.DatabaseLogger;
            ...
            for i = 1:length(states)
                ...
                for j = 1:length(names)
                    ...
                    if isa(data, 'edu.jiangnan.fmme.util.SparseMat')
                        if k == 1
    ...
    methods (Static)
        ...
        function data = allocateData(dataType, sz)
            ...
            switch dataType
                case 'edu.jiangnan.fmme.util.SparseMat', data = SparseMat([], [], sz);
                case 'edu.jiangnan.fmme.util.CircularSparseMat',  data = CircularSparseMat([], [], sz, 1);
    ...
    methods (Access = protected)
        ...
        function saveOptionsHelper(this, options, module)
            ...
            for i = 1:length(optionNames)
                value = edu.jiangnan.fmme.io.jsonFormat(options.(optionNames{i}));
        ...
        function saveFittedConstantsHelper(this, module)
            import edu.jiangnan.fmme.io.jsonFormat;
    ...
    methods (Static)
        ...
        function value = loadOptions(sim, database, simulationWID)
            ...
            for i = 1:length(result.WID)
                if isempty(result.Module{i})
                    value.(result.Name{i}) = edu.jiangnan.fmme.io.jsonParse(result.Value{i});
                elseif result.Module{i}(1) == 'S'
                    value.states.(result.Module{i}(7:end)).(result.Name{i}) = edu.jiangnan.fmme.io.jsonParse(result.Value{i});
                else
                    value.processes.(result.Module{i}(9:end)).(result.Name{i}) = edu.jiangnan.fmme.io.jsonParse(result.Value{i});
                end
            end
        end
        ...
        function value = loadRandStreamStates(~, database, simulationWID)
            ...
            for i = 1:length(result.WID)
                if isempty(result.Module{i})
                    value.simulation = edu.jiangnan.fmme.io.jsonParse(result.Value{i});
                elseif result.Module{i}(1) == 'S'
                    value.states.(result.Module{i}(7:end)) = edu.jiangnan.fmme.io.jsonParse(result.Value{i});
                else
                    value.processes.(result.Module{i}(9:end)) = edu.jiangnan.fmme.io.jsonParse(result.Value{i});
                end
            end
        end
        ...
        function states = loadTimecourses(sim, database, simulationWID)
            ...
            for i = 1:length(result.WID)
                states.(result.Module{i}(7:end)).(result.Name{i}) = edu.jiangnan.fmme.io.jsonParse(result.Value{i});
            end
        end
    end
end