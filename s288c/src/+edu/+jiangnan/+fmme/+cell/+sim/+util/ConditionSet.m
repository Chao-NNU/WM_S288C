...
classdef ConditionSet
    ...
    methods (Static = true)
        ...
        function generateConditionSet(sim, metadata, conditions, fileName)
            ...
            fprintf(fid, '<!-- Autogenerated by %s at %s -->\n',  'edu.jiangnan.fmme.cell.sim.util.ConditionSet', datestr(now, 31));
                ...
                if isfield(conditions(i), 'options') && isstruct(conditions(i).options)
                    ...
                    for j = 1:numel(fields)
                        fprintf(fid, '            <option name="%s" value="%s"/>\n', ...
                            fields{j}, edu.jiangnan.fmme.io.jsonFormat(conditions(i).options(fields{j})));
                    end
                        ...
                        for j = 1:numel(fields)
                            ...
                            for k = 1:numel(subfields)
                                fprintf(fid, '            <option state="%s" name="%s" value="%s"/>\n', ...
                                    ...
                                    edu.jiangnan.fmme.io.jsonFormat(conditions(i).options.states.(fields{j}).(subfields{k})));
                            end
                        end
                    end
                        ...
                        for j = 1:numel(fields)
                            ...
                            for k = 1:numel(subfields)
                                fprintf(fid, '            <option process="%s" name="%s" value="%s"/>\n', ...
                                    fields{j}, subfields{k}, ...
                                    edu.jiangnan.fmme.io.jsonFormat(conditions(i).options.processes.(fields{j}).(subfields{k})));
                            end
                        end
                    end
                            ...
                            for k = 1:numel(subfields)
                                if isscalar(conditions(i).parameters.states.(fields{j}).(subfields{k}))
                                    fprintf(fid, '            <parameter state="%s" name="%s" value="%s"/>\n', ...
                                        fields{j}, subfields{k}, ...
                                        edu.jiangnan.fmme.io.jsonFormat(conditions(i).parameters.states.(fields{j}).(subfields{k})));
                                else
                            ...
                            for k = 1:numel(subfields)
                                if isscalar(conditions(i).parameters.processes.(fields{j}).(subfields{k}))
                                    fprintf(fid, '            <parameter process="%s" name="%s" value="%s"/>\n', ...
                                        fields{j}, subfields{k}, ...
                                        edu.jiangnan.fmme.io.jsonFormat(conditions(i).parameters.processes.(fields{j}).(subfields{k})));
                                else
    ...
    methods (Static)
        function data = parseConditionSet(sim, fileName)
            ...
            edu.jiangnan.fmme.cell.sim.util.ConditionSet.validateConditionSet(xml);
            ...
            for i = 1:xml.getChildNodes.getLength
                ...
                switch char(child.getNodeName)
                    ...
                    case 'conditions'
                        ...
                        edu.jiangnan.fmme.cell.sim.util.ConditionSet.validateConditions(child);
                    otherwise
            ...
            for i = 1:xml.getChildNodes.getLength
                ...
                switch char(child.getNodeName)
                    ...
                    case 'condition'
                        ...
                        edu.jiangnan.fmme.cell.sim.util.ConditionSet.validateCondition(child);
                    otherwise
                        throw(MException('ConditionSet:invalidXML','invalid XML'));
                end
            end
        ...
        function validateCondition(xml)
            ...
            for i = 1:xml.getChildNodes.getLength
                ...
                switch char(child.getNodeName)
                    ...
                    case 'options'
                        edu.jiangnan.fmme.cell.sim.util.ConditionSet.validateOptions(child);
                    case 'parameters'
                        edu.jiangnan.fmme.cell.sim.util.ConditionSet.validateParameters(child);
                    case 'perturbations'
                        edu.jiangnan.fmme.cell.sim.util.ConditionSet.validatePerturbations(child);
                    otherwise
                        throw(MException('ConditionSet:invalidXML','invalid XML'));
                end
            end
        ...
        function validateOptions(xml)
            for i = 1:xml.getChildNodes.getLength
                ...
                switch char(child.getNodeName)
                    case {'#comment', '#text'}
                    case 'option'
                        edu.jiangnan.fmme.cell.sim.util.ConditionSet.validateOption(child);
                    otherwise
                        throw(MException('ConditionSet:invalidXML','invalid XML'));
                end
            end
        ...
        function validateParameters(xml)
            for i = 1:xml.getChildNodes.getLength
                ...
                switch char(child.getNodeName)
                    case {'#comment', '#text'}
                    case 'parameter'
                        edu.jiangnan.fmme.cell.sim.util.ConditionSet.validateParameter(child);
                    otherwise
                        throw(MException('ConditionSet:invalidXML','invalid XML'));
                end
            end
        ...
        function validatePerturbations(xml)
            for i = 1:xml.getChildNodes.getLength
                ...
                switch char(child.getNodeName)
                    case {'#comment', '#text'}
                    case 'perturbation'
                        edu.jiangnan.fmme.cell.sim.util.ConditionSet.validatePerturbation(child);
                    otherwise
                        throw(MException('ConditionSet:invalidXML','invalid XML'));
                end
            end
