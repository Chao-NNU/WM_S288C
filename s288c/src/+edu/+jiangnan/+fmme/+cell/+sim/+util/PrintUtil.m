    ...
    methods (Static)                    
        function printToStdIO(content, colLabels, options)
            if exist('options', 'var') && isstruct(options) && isfield(options, 'indentation')
                content = edu.jiangnan.fmme.cell.sim.util.PrintUtil.applyIndentation(content, options.indentation);
            end
            
            %write file
            colWidths = edu.jiangnan.fmme.cell.sim.util.PrintUtil.calcColWidths(content, colLabels);
        ...
        function printToFile(content, colLabels, fileName, varargin)
            ...
            if ismember(fileName(find(fileName == '.', 1, 'last')+1 : end), {'xls','xlsx'})
                if ispc
                    edu.jiangnan.fmme.cell.sim.util.PrintUtil.printToExcel(content, colLabels, fileName, varargin{:});
                    return;
                else
