...
classdef FtsZRing < edu.jiangnan.fmme.cell.sim.CellState
    ...
    methods
        function result = get.numEdges(this)
            g = this.geometry;
            if g.daughterwidth == 0
                result = 0;
                return;
            end
            
            result = this.calcNumEdges(g.daughterwidth, this.filamentLengthInNm);
        end
    end
    
    methods (Static)
        function result = calcNumEdges(daughterwidth, filamentLengthInNm)
            result = floor(pi / asin(filamentLengthInNm * 1e-9 / daughterwidth));
        end
    end
end
