...
classdef ProteinMonomer < edu.jiangnan.fmme.cell.sim.MoleculeCountState
    %Constants
    properties (Constant)
    ...
    methods
        function notUpdatingProteins = updateExternalState(this, deltaProteins, proteinIsDegraded)
			...
			for i = 16
				[posStrnds{i}, proteins{i}] = find(c.monomerBoundSites{i});
				...
				releasedMonomers{i} = c.setRegionProteinUnbound(i, ...
					chrReleasePosStrnds{i}, 1, idxs, [], ...
					false, false, false, proteinIsDegraded);
				if ~isequal(deltaBoundProteins(idxs), releasedMonomers{i})
