...
classdef KnowledgeBaseObject < handle
    properties
        ...
        knowledgeBase = edu.jiangnan.fmme.cell.kb.KnowledgeBase.empty(0, 0);
        references    = edu.jiangnan.fmme.cell.kb.Reference.empty(0, 0);
    end
    ...
    methods
        ...
        function classAndIndexs = serializeLinksHelper(~, vals)
            if isa(vals, 'edu.jiangnan.fmme.cell.kb.KnowledgeBaseObject')
        ...
        function vals = deserializeLinksHelper(~, classAndIndexs, all_objs)
            if nargin >= 3 && iscell(classAndIndexs) && isequal(size(classAndIndexs), [1 2]) && isequal(classAndIndexs{1}(1:min(26, end)), 'edu.jiangnan.fmme.cell.kb.')
                if ~iscell(all_objs)
                    all_objs = {all_objs};
                end
