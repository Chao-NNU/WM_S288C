...
classdef ProteinMonomer < edu.jiangnan.fmme.cell.kb.Polymer & edu.jiangnan.fmme.cell.kb.Protein
    properties
        gene                             = edu.jiangnan.fmme.cell.kb.Gene.empty(0,0);
        geneCompartments                 = edu.jiangnan.fmme.cell.kb.Compartment.empty(0,0);
        
        prostheticGroups                 = edu.jiangnan.fmme.cell.kb.Metabolite.empty(0,0);
        prostheticGroupCompartments      = edu.jiangnan.fmme.cell.kb.Compartment.empty(0,0);
        prostheticGroupCoefficients      = [];
        
        chaperoneSubstrates              = edu.jiangnan.fmme.cell.kb.ProteinMonomer.empty(0,0);
        chaperoneCompartments            = edu.jiangnan.fmme.cell.kb.Compartment.empty(0,0);
        ...
        regulatedTranscriptionUnits      = edu.jiangnan.fmme.cell.kb.TranscriptionUnit.empty(0,0);
        
        compartment                      = edu.jiangnan.fmme.cell.kb.Compartment.empty(0,0);
        
        stimuliRegulators                = edu.jiangnan.fmme.cell.kb.Stimuli.empty(0,0);
        metaboliteRegulators             = edu.jiangnan.fmme.cell.kb.Metabolite.empty(0,0);
        proteinMonomerRegulators         = edu.jiangnan.fmme.cell.kb.ProteinMonomer.empty(0,0);
        proteinComplexRegulators         = edu.jiangnan.fmme.cell.kb.ProteinComplex.empty(0,0);
    ...
    methods
		%{
        function this = ProteinMonomer(knowledgeBase, wid, wholeCellModelID, name, sequence, cdssequence, halfLife, molecularWeight,...
		...
		%}		
		function this = ProteinMonomer(knowledgeBase, wid, wholeCellModelID, name, halfLife,...
                topology, activeSite, metalBindingSite, dnaFootprint, dnaFootprintBindingStrandedness, dnaFootprintRegionStrandedness, molecularInteraction, ...
                chemicalRegulation, subsystem, generalClassification, proteaseClassification, ...
                transporterClassification, ...
                nTerminalMethionineCleavage, signalSequenceType, signalSequenceLocation, signalSequenceLength, ...
            ...
            this = edu.jiangnan.fmme.cell.kb.ProteinMonomer.empty(size(wid, 1), 0);
            this(size(wid, 1), 1) = edu.jiangnan.fmme.cell.kb.ProteinMonomer;
            for i = 1:size(wid, 1)
                this(i, 1).idx = i;
				...
				%this(i, 1).molecularWeight = molecularWeight(i);
        ...
        function serializeLinks(this)
            for i = 1:numel(this)
                ...
                serializeLinks@edu.jiangnan.fmme.cell.kb.Polymer(this(i));
                serializeLinks@edu.jiangnan.fmme.cell.kb.Protein(this(i));
            end
        end
        ...
        function deserializeLinks(this, kb)
            for i = 1:numel(this)
                ...
                deserializeLinks@edu.jiangnan.fmme.cell.kb.Polymer(this(i), kb);
                deserializeLinks@edu.jiangnan.fmme.cell.kb.Protein(this(i), kb);
            end
        end
		...
		function value = get.sequence(this)
            ...
            %calculate
			value = nt2aa(this.gene.cdsSequence,'GeneticCode',this.knowledgeBase.translationTable);
...
%             end
            %value = 20 * 60 * 60;
			value = 8.8 * 2 * 60 * 60; % Median halflife of protein for Saccharomyces cerevisiae is 8.8 hours
        end
    end
end