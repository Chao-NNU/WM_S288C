...
classdef Metabolite < edu.jiangnan.fmme.cell.kb.PhysicalObject
    properties
        reactions           = edu.jiangnan.fmme.cell.kb.Reaction.empty(0, 0);

        coenzymeReactions   = edu.jiangnan.fmme.cell.kb.Reaction.empty(0, 0);

        biomassCompartments = edu.jiangnan.fmme.cell.kb.Compartment.empty(0, 0);
        biomassCoefficients = [];

        mediaCompartments   = edu.jiangnan.fmme.cell.kb.Compartment.empty(0, 0);
    ...
    methods
        function this = Metabolite(knowledgeBase, wid,wholeCellModelID, name, ...
            ...
            this = edu.jiangnan.fmme.cell.kb.Metabolite.empty(size(wid,1),0);
            this(size(wid,1),1) = edu.jiangnan.fmme.cell.kb.Metabolite;
            for i = 1:size(wid,1)
                this(i, 1).idx = i;
                ...
                this(i,1).hydrophobic = strcmp(hydrophobic, 'Y');%hydrophobic(i);                
                this(i,1).pKa = edu.jiangnan.fmme.util.parseDoubles(', ', pKa{i});
                this(i,1).pI = pI(i);
                this(i,1).logP = logP(i);
                this(i,1).logD = logD(i);
                this(i,1).volume = volume(i);
                this(i,1).molecularWeight = molecularWeight(i);%new added
				this(i,1).exchangeLowerBound = NaN;
				this(i,1).exchangeUpperBound = NaN;
       ...
       %{ 
        ignored as the molecularWeight was generated by manual collection not based on the calculate results
        function value = get.molecularWeight(this)
            ...
            for i = 1:length(fields)
                if ~isfield(edu.jiangnan.fmme.util.ConstantUtil.elements,fields{i})
                    value = 0;
                    return;
                end
                value = value+...
                    this.empiricalFormula.(fields{i})*edu.jiangnan.fmme.util.ConstantUtil.elements.(fields{i});
            end
