...
classdef MySQLDatabase < edu.jiangnan.fmme.db.Database
    ...
    methods
        ...
        function this = MySQLDatabase(varargin)
            this = this@edu.jiangnan.fmme.db.Database(varargin{:});
            this.nullValue = 0;
            this.open();            
        end
        ...
        function prepareStatement(this, sql, varargin)
            ...
            [startIndex, tokenStr] = regexp(sql, '"{(S|Sn|Si|M|B|F|uF)}"', 'start', 'tokens');%find the location of variable
            sql = regexprep(sql, '"{(S|Sn|Si|M|B|F|uF)}"', '?');%replace {(S|Sn|Si|M|B|F|uF)} as ?
            ...
            for i = 1:length(startIndex)
                switch tokenStr{i}{1}
                    case 'S',     this.sqlStatement.setString(i, java.lang.String(varargin{i}));%provide value for variable
        ...
        function delete(this)
            this.close();
            this.delete@edu.jiangnan.fmme.db.Database();
        end
