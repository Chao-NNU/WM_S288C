...
for i = 1:numel(sim.states)
    ...
    for j = 1:numel(propIDs)
        ...
        if isa(sim.states{i}, 'edu.jiangnan.fmme.cell.sim.state.Chromosome') && isa(result, 'SparseMat')
            ...
            if ...
                    (isa(sim.states{i}, 'edu.jiangnan.fmme.cell.sim.MoleculeCountState') && strcmp(propIDs{j}, 'counts')) || ...
                    (isa(sim.states{i}, 'edu.jiangnan.fmme.cell.sim.state.Stimulus') && strcmp(propIDs{j}, 'values'))
                result = sum(result, 2);
            end
