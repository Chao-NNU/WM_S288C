    ...
    methods (Static = true)
        function [simDir, revision, sim] = getSimulation(simDir)
            ...
            if ~((isunix && simDir(1) == '/') || (ispc && simDir(2) == ':'))
                simDir = [edu.jiangnan.fmme.cell.sim.util.SimulationDiskUtil.getBaseDir() filesep simDir];
            end
        ...
        function [simGroupDir, revision, sim] = getLatestSimulationGroup()
            ...
            baseDir = edu.jiangnan.fmme.cell.sim.util.SimulationDiskUtil.getBaseDir(); %#ok<*PROP>
        ...
        function value = getNumSimulations(simBatchDir)
			if (isunix && simBatchDir(1) == '/') || (ispc && simBatchDir(2) == ':')
				...
				baseDir = [edu.jiangnan.fmme.cell.sim.util.SimulationDiskUtil.getBaseDir filesep simBatchDir];
			end
            files = dir(baseDir);
