...
classdef FitConstants < handle
    ...
    methods
        function this = FitConstants(sim, options)
            ...
            this.nRNAWtFracs = 7; %change form 6 to 7
            ...
            [~, this.monomerIndex_gyrase] = ismember({'YPL024W_MONOMER', 'YMR190C_MONOMER','YLR234W_MONOMER'}, pm.wholeCellModelIDs(pm.matureIndexs));
            [~, this.monomerIndex_topoisomeraseI] = ismember({'YOL006C_MONOMER'}, pm.wholeCellModelIDs(pm.matureIndexs));
			%[~, this.complexIndex_topoisomeraseIV] = ismember({'YNL088W_MONOMER'}, pm.wholeCellModelIDs(pm.matureIndexs));
            [~, this.monomerIndex_ORC] = ismember({'YML065W_MONOMER'}, pm.wholeCellModelIDs(pm.matureIndexs));
            %[~, this.monomerIndex_FtsZ] = ismember({'YDL126C_MONOMER'}, pm.wholeCellModelIDs(pm.matureIndexs));
			[~, this.monomerIndex_FtsZ] = ismember({'YLR229C_MONOMER'}, pm.wholeCellModelIDs(pm.matureIndexs));%CDC42
            
            this.rnaIndex_ORC = find(r.matureRNAGeneComposition(g.mRNAIndexs(this.monomerIndex_ORC), :));
            ...
            %initiation duration
            this.ORCCnt = 573; %Kulak NA, et al. (2014)
			%this.ORCCnt = 2300; %Average concentration of ORC according to Uniprot database
        ...
        function this = run(this)
            %import classes
            import edu.jiangnan.fmme.cell.sim.constant.Condition;
        ...
        function [Aineq, bineq, Aeq, beq] = linConstraintFunc(this, paramVec)
            ...
            Aeq(end-2, this.rnaExpIdxs(this.rnaIndex_ORC)) = 1;
            beq(end-2) = rnaExp(this.rnaIndex_ORC);
        ...
        function paramVec = fitHeuristically(this, paramVec)
            ...
            for i = 1:this.maxIter
                ...
                minMonCnt = max(minMonCnt, max(sum(pc.proteinComplexComposition(g.mRNAIndexs, :, :), 3), [], 2) * pc.minimumAverageExpression);
                minRNACnt = min(minRNACnt, maxRNACnt);
                minMonCnt = min(minMonCnt, maxMonCnt);
                
                %ribosomal proteins
                minMonCnt = max(minMonCnt, sum(pc.proteinComplexComposition(g.mRNAIndexs, pc.ribosome80SIndexs, :), 3) * 200);
                
                %DnaA, FtsZ
                minMonCnt(this.monomerIndex_ORC) = this.ORCCnt;
                minMonCnt(this.monomerIndex_FtsZ) = this.ftsZCnt;
                maxMonCnt(this.monomerIndex_ORC) = this.ORCCnt;
        ...
        function [lclBmComp, lclBmProd, lclByProd, unaccECons, minRNAExp, minMonExp, maxRNAExp, maxMonExp] = ...
			...
			%% import classes
            import edu.jiangnan.fmme.cell.sim.util.MetaboliteUtil
            import edu.jiangnan.fmme.util.ComputationUtil;
            import edu.jiangnan.fmme.util.ConstantUtil;
            ...
            if mass.dryWeightFractionDNA
				for i = 1:16
					dnmpComp{i} = [getBaseCounts(c.sequence{i}); nnz(c.damagedBases{i} == m.m6ADIndexs)];
					dnmpComp{i}(1) = dnmpComp{i}(1) - nnz(c.damagedBases{i} == m.m6ADIndexs);
					
					chrWt{i} = dnmpComp{i}' * (m.molecularWeights([m.dnmpIndexs; m.getIndexs({'M6DAMP'})]) - ...
							   ConstantUtil.elements.O - ConstantUtil.elements.H) / ConstantUtil.nAvogadro;
				end
				chrWt = sum(cell2mat(chrWt));
				dnmpComp = dnmpComp{1} + dnmpComp{2} + dnmpComp{3} + dnmpComp{4} + dnmpComp{5} + dnmpComp{6}...
						 + dnmpComp{7} + dnmpComp{8} + dnmpComp{9} + dnmpComp{10} + dnmpComp{11} + dnmpComp{12}...
						 + dnmpComp{13} + dnmpComp{14} + dnmpComp{15} + dnmpComp{16};	
				...
				dnmpComp = [];
				for i = 1:16
					dnmpComp{i} = getBaseCounts(c.sequence{i});
				end
				dnmpComp = dnmpComp{1} + dnmpComp{2} + dnmpComp{3} + dnmpComp{4} + dnmpComp{5} + dnmpComp{6}...
						 + dnmpComp{7} + dnmpComp{8} + dnmpComp{9} + dnmpComp{10} + dnmpComp{11} + dnmpComp{12}...
						 + dnmpComp{13} + dnmpComp{14} + dnmpComp{15} + dnmpComp{16};
            ...
            %constants
            constants = edu.jiangnan.fmme.util.StructUtil.catstruct(...
            ...
            minMonExp(this.monomerIndex_ORC) = this.ORCCnt;
            minMonExp(this.monomerIndex_FtsZ) = this.ftsZCnt;
            
            maxMonExp(this.monomerIndex_ORC) = this.ORCCnt;
        ...
        function [freeRnas, freeMons, freeCpxs, ...
            ...
            ignoreCpxIdxs = [
                ...
                pc.getIndexs({'RIBOSOME_40S'; 'RIBOSOME_40S_IF2'; 'RIBOSOME_60S'; 'RNA_POLYMERASE_III'})];
            ...
            [subs2Nets, cpxs2Nets, nets] = edu.jiangnan.fmme.util.findNonInteractingRowsAndColumns(...
            ...
            totCpxs(pc.getIndexs({'YIL026C_YDL003W_YPR007C_YFL008W_YJL074C_5MER_ADP'; 'YIL026C_YDL003W_YPR007C_YFL008W_YJL074C_PENTAMER'})) = ...
                + totCpxs(pc.getIndexs({'YIL026C_YDL003W_YPR007C_YFL008W_YJL074C_5MER_ADP'; 'YIL026C_YDL003W_YPR007C_YFL008W_YJL074C_PENTAMER'})) ...
                + [1; -1] * totCpxs(pc.getIndexs('YIL026C_YDL003W_YPR007C_YFL008W_YJL074C_PENTAMER'));
            
            idxs = [sim.process('FtsZPolymerization').enzymeComplexGlobalIndexs; pc.getIndexs('YDL126C_9MER_GTP')];
            ...
            idx = sim.process('ReplicationInitiation').enzymeGlobalIndexs(sim.process('ReplicationInitiation').enzymeIndexs_ORC_1mer_ATP);
			totCpxs(idx) = sum(totCpxs(idxs));
            totCpxs(setdiff(idxs, idx)) = 0;
        ...
        function applyParameterVectorToSimulation(this, paramVec)
			...
			for i = 1:16
            if ~isempty(c.polymerizedRegions{i}) && ~isempty(tr)
				tfFoldChange{i} = tr.calcBindingProbabilityFoldChange(i, tr.bindTranscriptionFactors(i));	
            else
                tfFoldChange{i} = ones(size(c.transcriptionUnitStartCoordinates{i}, 1), 2);
            end
			
            if ~isempty(sc)
				scProbFoldChange{i} = sc.calcRNAPolymeraseBindingProbFoldChange(i, [1 1], ...
									  c.sequenceLen(i),...
									  c.sequenceLen(i) / c.relaxedBasesPerTurn(i) * (1 + c.equilibriumSuperhelicalDensity),...
									  c.transcriptionUnitStartCoordinates{i},...
									  c.relaxedBasesPerTurn(i));
            else
                scProbFoldChange{i} = ones(size(c.transcriptionUnitStartCoordinates{i}, 1), 2);
            end
			
			t.transcriptionUnitBindingProbabilities{i} = ...
                t.transcriptionUnitBindingProbabilities{i} ...
                ./ tfFoldChange{i}(:, 1) ...
                ./ scProbFoldChange{i}(:, 1) ...
                ./ this.calcEffectiveMeanTranscriptionUnitCopyNumbers(i);
			
            %normalize RNA polymerase binding probability
            t.transcriptionUnitBindingProbabilities{i} = ...
                t.transcriptionUnitBindingProbabilities{i} / ...
                sum(t.transcriptionUnitBindingProbabilities{i});
			end
            ...
            nPols = totCpxs(t.enzymeGlobalIndexs(t.enzymeIndexs_rnaPolymerase2));
			
			for i =1:16
			t.stateTransitionProbabilities{i} = t.calcStateTransitionProbabilities(...
                nPols, ntpProd, t.transcriptionUnitBindingProbabilities{i}, i);
			end
        end
        ...
        function aaCounts = getMonomerAACounts(this)
            ...
            aaCounts(:, m.getIndexs('CYS_L') == m.aminoAcidIndexs) = ...
                aaCounts(:, m.getIndexs('CYS_L') == m.aminoAcidIndexs) + ...
        ...
        function value = calcEffectiveMeanTranscriptionUnitCopyNumbers(this, i)
            sim = this.simulation;
            c = sim.state('Chromosome');
            time = sim.state('Time');
            value = (...
                + 1 * time.replicationInitiationDuration ...
                + (1 + abs(c.transcriptionUnitStartCoordinates{i} + c.transcriptionUnitLengths{i} - 1 - c.sequenceLen(i)/2) / (c.sequenceLen(i)/2)) * time.replicationDuration ...
                + 2 * time.cytokinesisDuration ...
                ) / time.cellCycleLength;
            
            iTU = sim.process('Transcription').transcriptionUnitIndexs_DnaAR12345Boxes{i};
