...
classdef SimulationEnsemble < handle
    ...
    methods
            ...
            for i = 1:length(this.selectedSimulations)
                ...
                elseif ~exist(summaryFileName, 'file')
                    ...
                    for j = 1:length(this.stateData.properties)
						...
						if size(summary.(this.summaryLogToEnsemble{ensemblePropIdxs(j), 2}),2) == 16
						results = double(summary.(this.summaryLogToEnsemble{ensemblePropIdxs(j), 2}){16}(this.summaryLogToEnsemble{ensemblePropIdxs(j), 3}, indStart:indStop));
						else
        ...
        function value = getTimeStamp(this)
            value = edu.jiangnan.fmme.cell.sim.util.SimulationDiskUtil.getSimulationTimeStamp([this.directory filesep '1']);
        end
    end
