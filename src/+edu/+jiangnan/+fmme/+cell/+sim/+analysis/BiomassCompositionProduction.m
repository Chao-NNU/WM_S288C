...
classdef BiomassCompositionProduction
    properties (Constant)
        ...
        dryWeightFractionIndex_Carbohydrate = 5;
        dryWeightFractionIndex_Vitamin      = 6;
        dryWeightFractionIndex_Ion          = 7;
    end
    
    methods (Static)
        function run(sim, fileName)
            import edu.jiangnan.fmme.cell.sim.analysis.BiomassCompositionProduction;
            import edu.jiangnan.fmme.cell.sim.util.PlotUtil;
            import edu.jiangnan.fmme.cell.sim.util.PrintUtil;
            
            [bmComp, bmProd, byProd] = edu.jiangnan.fmme.cell.sim.analysis.BiomassCompositionProduction.getResourceRequirements(sim);
    ...
    methods (Static)
        function plotBiomassComposition(sim, axesHandle, bmComp)
            ...
            %pie chart
			A = cellfun(@(value, label) {label sprintf('(%.1f%%)', value * 100)}, num2cell(values), labels, 'UniformOutput', false);
			for i = 1:length(A)
			B{i} = char(A{i});
			end
			%{
            ...
            values = sum(bmComp(idxs, :), 2);
            values = values / sum(values);
            labels = num2cell(edu.jiangnan.fmme.cell.kb.ProteinMonomer.bases);
