...
classdef Replication
    ...
    methods (Static)
        function run(outputFileName)
            import edu.jiangnan.fmme.cell.sim.analysis.Replication;
            ...
            if nargin == 0, [axesHandle, figHandle] = edu.jiangnan.fmme.cell.sim.util.PlotUtil.newAxesHandle(); end
    ...
    methods (Static)
        ...
        function [time, primases, leadingPols, laggingPols, ssbs, ligases, segregation] = simulateReplication(seed)            
            ...
            m = edu.jiangnan.fmme.cell.sim.ProcessFixture.load(edu.jiangnan.fmme.cell.sim.process.Replication([], 'Replication'));
            m.verbosity = 0;
            if ~exist('seed', 'var')
                seed = 1;
            end
