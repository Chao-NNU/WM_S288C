...
classdef CellState
    ...
    methods (Static)
        function run(sim, fileName)
            import edu.jiangnan.fmme.cell.sim.analysis.CellState;
            import edu.jiangnan.fmme.cell.sim.util.PlotUtil;
        ...
        function [content, colLabels, indentation] = state(sim)
            ...
            content = [content;{
                0 'Shape' 'Cyclinder with hemispherical caps' []
                    %1 'Width' s.width 'm'
					1 'MotherWidth' s.motherwidth 'm'
					1 'SaughterWidth' s.daughterwidth 'm'
					1 'MotherLength' s.motherlength 'm'
					1 'SaughterLength' s.daughterlength 'm'					
            ...
            content = [content;{
                0 'Metabolites' [] []
                    ...
                    1 'dNTPs' [] []
                        ...
                        2 'Incorporated into DNA' (1 - sum(freedNTPs) / (sum(freedNTPs) + collapse(c.polymerizedRegions{16})))*100 '%'
                    1 'NTPs' [] []
			...
			for i = 1
            dnaABoxBound{i} = ...
                ri.calculateDnaABoxStatus(i) == ri.dnaABoxStatus_DnaAATPBound | ...
                ri.calculateDnaABoxStatus(i) == ri.dnaABoxStatus_DnaAADPBound;
            [polATPs{i}, polADPs{i}] = ri.calculateDnaAR1234Polymerization(i);
            oriCComplexSize{i} = ri.calculateDnaAR1234ComplexSize(i, polATPs, polADPs);
            [~, boundMonomers{i}] = find(c.monomerBoundSites{i});
            [~, boundComplexs{i}] = find(c.complexBoundSites{i});
			
            %occupiedBases{i} = sum(c.monomerDNAFootprints(boundMonomers{i}) + c.complexDNAFootprints(boundComplexs{i}));
			occupiedBases{i} = sum(c.monomerDNAFootprints(boundMonomers{i}));
                        
            methylatedRMSites{i} = nnz(c.damagedBases{i}([
                dr.RM_MunI_RecognitionSites{i}(:, dr.RM_MunI_MethylatedPositions(1))   ones(size(dr.RM_MunI_RecognitionSites{i}, 1), 1);
                dr.RM_MunI_RecognitionSites{i}(:, dr.RM_MunI_MethylatedPositions(2)) 2*ones(size(dr.RM_MunI_RecognitionSites{i}, 1), 1);
                dr.RM_MunI_RecognitionSites{i}(:, dr.RM_MunI_MethylatedPositions(1)) 3*ones(size(dr.RM_MunI_RecognitionSites{i}, 1), 1);
                dr.RM_MunI_RecognitionSites{i}(:, dr.RM_MunI_MethylatedPositions(2)) 4*ones(size(dr.RM_MunI_RecognitionSites{i}, 1), 1);
                ])) / 2;
            polymerizedRMSites{i} = sum(c.isRegionPolymerized(i,[
                dr.RM_MunI_RecognitionSites{i}(:, 1)   ones(size(dr.RM_MunI_RecognitionSites{i}, 1), 1);
                dr.RM_MunI_RecognitionSites{i}(:, 1) 2*ones(size(dr.RM_MunI_RecognitionSites{i}, 1), 1);
                dr.RM_MunI_RecognitionSites{i}(:, 1) 3*ones(size(dr.RM_MunI_RecognitionSites{i}, 1), 1);
                dr.RM_MunI_RecognitionSites{i}(:, 1) 4*ones(size(dr.RM_MunI_RecognitionSites{i}, 1), 1);
                ], 6, false)) / 2;
            
            content = [content;{
                0 'No. chromosomes' collapse(c.polymerizedRegions{i})/c.sequenceLen(i) [];
                    1 'Bases' collapse(c.polymerizedRegions{i}) 'nt';
                    1 'Linking number' collapse(c.linkingNumbers{i})/2 'Links';
                    1 'Superhelical density', (collapse(c.linkingNumbers{i})  - collapse(c.linkingNumbers_minFreeEnergy{i})) / collapse(c.linkingNumbers_minFreeEnergy{i}) [];
                    1 'Bound proteins' nnz(c.monomerBoundSites{i}) + nnz(c.complexBoundSites{i}) 'molecules'
                        2 'Monomers' nnz(c.monomerBoundSites{i}) 'molecules'
                            3 'GntR' nnz(c.monomerBoundSites{i} == pm.getIndexs({'YKL113C_MONOMER'})) 'molecules'
                        2 'Complexes' nnz(c.monomerBoundSites{i}) 'molecules'
                            3 'DnaA' nnz(dnaABoxBound{i}) 'molecules'
                                4 '15-mer' nnz(dnaABoxBound{i}(ri.ACSBoxIndexs_15mer{i}, :)) 'molecules'
                                4 '17-mer' nnz(dnaABoxBound{i}(ri.ACSBoxIndexs_17mer{i}, :)) 'molecules'
                                4 '33-mer' nnz(dnaABoxBound{i}(ri.ACSBoxIndexs_33mer{i}, :)) 'molecules'
                                4 'OriC Complex' oriCComplexSize{i}(1) 'molecules'
                                4 'R1' nnz(dnaABoxBound{i}(ri.dnaABoxIndexs_R12345{i}(1), :)) 'molecules'
                                4 'R2' nnz(dnaABoxBound{i}(ri.dnaABoxIndexs_R12345{i}(2), :)) 'molecules'
                                4 'R3' nnz(dnaABoxBound{i}(ri.dnaABoxIndexs_R12345{i}(3), :)) 'molecules'
                                4 'R4' nnz(dnaABoxBound{i}(ri.dnaABoxIndexs_R12345{i}(3), :)) 'molecules'
                                4 'R5' nnz(dnaABoxBound{i}(ri.dnaABoxIndexs_R12345{i}(5), :)) 'molecules'
                            3 'Gyrase' nnz(c.complexBoundSites{i} == pc.getIndexs({'DNA_GYRASE'})) 'molecules'
                            3 'LuxR' nnz(c.complexBoundSites{i} == pc.getIndexs({'MG_428_DIMER'})) 'molecules'
                            3 'SMC' nnz(c.complexBoundSites{i} == pc.getIndexs({'YIL026C_YDL003W_YPR007C_YFL008W_YJL074C_PENTAMER_ADP'})) 'molecules'
                                4 'Average spacing' collapse(c.polymerizedRegions{i})/2 / nnz(c.complexBoundSites{i} == pc.getIndexs({'YIL026C_YDL003W_YPR007C_YFL008W_YJL074C_PENTAMER_ADP'})) 'nt'
                        2 'Occupied' occupiedBases{i} 'nt'
                        2 'Occupied' occupiedBases{i} / collapse(c.polymerizedRegions{i}) / 2 * 100 '%'
                    1 'Damage' [] []
                        2 'Gap sites' nnz(c.gapSites{i}) 'nt'
                        2 'Abasic sites' nnz(c.abasicSites{i}) 'nt'
                        2 'Damaged bases' nnz(c.damagedBases{i}) 'nt'
                            3 'm6AD' nnz(c.damagedBases{i} == c.metabolite.m6ADIndexs) 'nt'
                            3 'methylated R/M sites' methylatedRMSites{i} 'nt'
                            3 'methylated R/M sites' methylatedRMSites{i}/polymerizedRMSites{i}*100 '%'
                        2 'Damaged sugar-phosphate' nnz(c.damagedSugarPhosphates{i}) 'nt'
                        2 'Strand breaks' nnz(c.strandBreaks{i}) 'nt'
                        2 'Intrastrand crosslinks' nnz(c.intrastrandCrossLinks{i}) 'nt'
                        2 'Holliday junctions' nnz(c.hollidayJunctions{i}) 'nt'
                }];
				end
            ...
            content = [content;{
                0 'RNA' sum(r.counts(:)) 'molecules'
                    ...
                    1 'rRNA' sum(matureRNAs(r.matureRRNAIndexs)) 'molecules'
						...
						2 '5.8S'  matureRNAs(r.matureRibosomalRRNAIndexs(2)) 'molecules'
                        2 '18S' matureRNAs(r.matureRibosomalRRNAIndexs(3)) 'molecules'
                        2 '25S' matureRNAs(r.matureRibosomalRRNAIndexs(4)) 'molecules'
                    ...
                    1 'Immature' nnz(sim.process('Transcription').transcripts.boundTranscriptionUnits{16}) 'molecules'
                    1 'Aborted' size(sim.state('Transcript').abortedTranscripts{16}, 1) 'molecules'
                }];
            ...
            ribosomalMonomerIdxs = sum(c.proteinComplexComposition(sim.gene.mRNAIndexs, c.getIndexs({'RIBOSOME_80S'}), :),3);
            ...
            content = [content;{
                0 'Protein' sum(m.counts(:)) + sum(c.counts(:)) 'molecules'
                    ...
                    1 'Bound' sum(sum(m.counts(m.boundIndexs, :)))+sum(sum(c.counts(c.boundIndexs, :))) 'molecules'
                        2 'Chromosome' nnz(sim.state('Chromosome').monomerBoundSites{1})+nnz(sim.state('Chromosome').complexBoundSites{1}) 'molecules'
                        2 'RNA polymerase' nnz(sim.process('Transcription').transcripts.boundTranscriptionUnits{1}) 'molecules'
                    ...
                    1 'Mitochondria' sum(m.counts(:, sim.compartment.mitochondriaIndexs))+ sum(c.counts(:, sim.compartment.mitochondriaIndexs)) 'molecules'
            ...
            rnaPolymeraseStateOccupancies = s.stateOccupancies{1}*100;
            content = [content;{
                0 'Transcription enzymes' [] []                
                    1 'RNA polymerase' t.enzymes(t.enzymeIndexs_rnaPolymerase2)+t.boundEnzymes(t.enzymeIndexs_rnaPolymerase2) 'molecules'
            ...
            content = [content;{
                0 'Translation enzymes' [] []
                    1 'Ribosome' [] []
                        2 'Ribosome 40S' totEnzymes(t.enzymeIndexs_ribosome40S) 'molecules'
                        2 'Ribosome 40S-IF2' totEnzymes(t.enzymeIndexs_ribosome40SIF2) 'molecules'
                        2 'Ribosome 60S' totEnzymes(t.enzymeIndexs_ribosome60S) 'molecules'
                        2 'Ribosome 80S' totEnzymes(t.enzymeIndexs_ribosome80S) 'molecules'
                            3 'Free' t.enzymes(t.enzymeIndexs_ribosome80S) 'molecules'
                            3 'Bound' t.boundEnzymes(t.enzymeIndexs_ribosome80S) 'molecules'
        ...
        function [content, colLabels, indentation] = processes(sim)
            ...
            rnaPolymeraseMaxNTPIncorporation = s.nActive{16} * t.rnaPolymeraseElongationRate;
            ...
            content = [content;{
                0 'Transcription' [] []
                    ...
                    1 'Min NTP' minNTP []
                    1 'Active RNA Polymerases' s.nActive{16} 'molecules'
    ...
    methods (Static)
        function plotStateOnChromosome(sim, axesHandle)
            ...
            %polymerized regions
            for i = 1:4
                [pos, lens] = find(c.polymerizedRegions{16}(:, i));
                [r, xoffset] = calcRadiusXOffset(i);
                for j = 1:size(pos, 1)
                    theta = 2*pi * (pos(j,1) + (0:lens(j)/360:lens(j)))/c.sequenceLen(16);
            ...
            for i = 1:2
                for j = 1:numel(c.transcriptionUnitWholeCellModelIDs)
                    if ~c.polymerizedTranscriptionUnits{16}(j, i); continue; end;
                    [r, xoffset] = calcRadiusXOffset(2*(i-1) + c.transcriptionUnitStrands{16}(j));
                    theta = 2*pi * (c.transcriptionUnitStartCoordinates{16}(j) + (0:c.transcriptionUnitLengths{16}(j)/360:c.transcriptionUnitLengths{16}(j))) / c.sequenceLen(16);
            ...
            %polymerized genes
            for i = 1:2
                for j = 1:498%numel(sim.gene.wholeCellModelIDs)
                    if ~c.polymerizedGenes{16}(j, i); continue; end;
                    [r, xoffset] = calcRadiusXOffset(2*(i-1) + sim.gene.strands(j));
                    theta = 2*pi * (sim.gene.startCoordinates(j) + (0:sim.gene.lengths(j)/360:sim.gene.lengths(j))) / c.sequenceLen(16);
            ...
            %bound proteins
            [monomerPosStrnds, monomers] = find(c.monomerBoundSites{16});
            [complexPosStrnds, complexs] = find(c.complexBoundSites{16});
            ...
            for i = 1:size(posStrnds, 1)
                ...
                theta = 2*pi * (posStrnds(i,1) + (0:lens(i)/360:lens(i))) / c.sequenceLen(16);
            ...
            %damage
            posStrnds = find(c.getDamagedSites(16,1,1,1,1,1,1,1));
            for i = 1:size(posStrnds, 1)
                [r, xoffset] = calcRadiusXOffset(posStrnds(i,2));
                theta = 2*pi * posStrnds(i,1) / c.sequenceLen(16);
            ...
            for i = 1:498%numel(sim.gene.wholeCellModelIDs)
            ...
            for i = 1:numel(monomers)
                ...
                theta = 2*pi * (sim.gene.startCoordinates(sim.gene.mRNAIndexs(i)) + sim.gene.lengths(sim.gene.mRNAIndexs(i))/2) / c.sequenceLen(16);
