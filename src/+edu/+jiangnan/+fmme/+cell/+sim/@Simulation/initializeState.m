...
import edu.jiangnan.fmme.util.RandStream;
...
for i=1:16
	chrComp{i} = [getBaseCounts(c.sequence{i}); 0];
end
		
if ~isempty(dnaRepair)
	for i = 1:16
    M6DAMPComp{i} = 2 * size(dnaRepair.RM_MunI_RecognitionSites{i}, 1);
    chrComp{i}(5) = M6DAMPComp{i};
    chrComp{i}(1) = chrComp{i}(1) - M6DAMPComp{i};
	end
end
...
if strcmp(this.macromoleculeStateInitialization, 'expected')
    ...
    [subs2Nets, cpxs2Nets, nets] = edu.jiangnan.fmme.util.findNonInteractingRowsAndColumns(pcComp);
    for i = 1:numel(nets)
        if size(nets{i}, 2) <= 1
            continue;
        end
...
this.state('Transcript').abortedTranscripts{i} = this.state('Transcript').abortedTranscripts{i}([], :);
end
