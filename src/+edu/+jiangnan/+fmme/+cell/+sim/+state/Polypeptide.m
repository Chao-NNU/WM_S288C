...
classdef Polypeptide < edu.jiangnan.fmme.cell.sim.CellState
    ...
    methods
        function value = get.totalBaseCounts(this)
            ...
            value = zeros(1, 20);% replaced 21 by 20
            ...
            for i = 1:size(this.boundMRNAs, 1)
                ...
                if ~isempty(sequence)
                    value = value + ProteinMonomer.computeBaseCount(sequence, 20, 1:20, true);
                end
            end
            
            abortedSeqs = this.abortedSequences;
            for i = 1:numel(abortedSeqs)
                value = value + ProteinMonomer.computeBaseCount(abortedSeqs{i}, 20, 1:20, true);
            end
        end
