...
classdef Ribosome < edu.jiangnan.fmme.cell.sim.CellState
    ...
    methods
        ...
        function storeObjectReferences(this, simulation)
            this.storeObjectReferences@edu.jiangnan.fmme.cell.sim.CellState(simulation);
    ...
    methods
        function releaseRibosome(this, nRibs, mRNAs)
            ...
            if ~isempty(ribIdxs) + ~isempty(mRNAIdxs) > 1
                idxs = unique([ribIdxs; mRNAIdxs]);
            else
                idxs = [ribIdxs; mRNAIdxs];
            end
            
            %abort peptides
            for i = 1:numel(idxs)
                if this.mRNAPositions(idxs(i)) == 0
                    continue;
                end
            ...
            if numel(idxs) - nRibs > 0
                this.complex.counts(this.complex.boundIndexs(this.complex.ribosome80SIndexs), this.complex.compartment.cytosolIndexs) = ...
                    + this.complex.counts(this.complex.boundIndexs(this.complex.ribosome80SIndexs), this.complex.compartment.cytosolIndexs) ...
                    - (numel(idxs) - nRibs);
                this.complex.counts(this.complex.matureIndexs([this.complex.ribosome40SIndexs; this.complex.ribosome60SIndexs]), this.complex.compartment.cytosolIndexs) = ...
                    + this.complex.counts(this.complex.matureIndexs([this.complex.ribosome40SIndexs; this.complex.ribosome60SIndexs]), this.complex.compartment.cytosolIndexs) ...
                    + (numel(idxs) - nRibs);
            end
            %{
