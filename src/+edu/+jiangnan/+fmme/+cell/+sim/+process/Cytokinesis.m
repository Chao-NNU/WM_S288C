...
classdef Cytokinesis < edu.jiangnan.fmme.cell.sim.Process & edu.jiangnan.fmme.cell.sim.ChromosomeProcessAspect
    ...
    properties
        ...
        enzymeWholeCellModelIDs = {              %whole cell model IDs of enzymes
            'YDL126C_9MER_GTP';      %cell division protein ftsZ polymer activated
            'YDL126C_9MER_GDP';      %cell division protein ftsZ polymer deactivated and bound to cell membrane
            'YDL126C_MONOMER_GDP';   %cell division protein ftsZ deactivated (monomer)
            'YDL126C_MONOMER_GTP'};  %cell division protein ftsZ activated (monomer)
    ...
    methods
        ...
        function initializeConstants(this, varargin)
            this.initializeConstants@edu.jiangnan.fmme.cell.sim.Process(varargin{:});
            this.initializeConstants@edu.jiangnan.fmme.cell.sim.ChromosomeProcessAspect(varargin{:});
        end
    end
    ...
    methods
        ...
        function [bmProd, byProd, minEnzExp, maxEnzExp] = calcResourceRequirements_LifeCycle(this, ~, ~)
            ...
            [~, gtpCost] = this.calcRequiredPinchingCycles(this.geometry.daughterwidth, this.ftsZRing.numFtsZSubunitsPerFilament, this.ftsZRing.filamentLengthInNm);
            ...
            minEnzExp(this.enzymeIndexs_ftsZ_GTP_polymer) = ...
                4 * this.ftsZRing.calcNumEdges(this.geometry.daughterwidth, this.ftsZRing.filamentLengthInNm);
        end
    ...
    %model helpers
    methods (Static)
        function result = calcNextPinchedDiameter(daughterwidth, filamentLengthInNm)
            import edu.jiangnan.fmme.cell.sim.state.FtsZRing;
            
            numEdges = FtsZRing.calcNumEdges(daughterwidth, filamentLengthInNm);
            ...
            result = newDiameter + (daughterwidth - flooredDiameter);
        ...
        function [nCycles, nGTP] = calcRequiredPinchingCycles(daughterwidth, numFtsZSubunitsPerFilament, filamentLengthInNm)
            import edu.jiangnan.fmme.cell.sim.process.Cytokinesis;
            import edu.jiangnan.fmme.cell.sim.state.FtsZRing;
            
            nCycles  = 0;
            nGTP     = 0;
            while daughterwidth > 0
                nCycles  = nCycles + 1;
                nGTP     = nGTP + 2 * numFtsZSubunitsPerFilament * FtsZRing.calcNumEdges(daughterwidth, filamentLengthInNm);
                daughterwidth = Cytokinesis.calcNextPinchedDiameter(daughterwidth, filamentLengthInNm);
            end
        end
    end
end
