...
classdef  DNADamage < edu.jiangnan.fmme.cell.sim.ReactionProcess & edu.jiangnan.fmme.cell.sim.ChromosomeProcessAspect
    ...
    methods
        ...
        function initializeConstants(this, knowledgeBase, simulation, varargin)
            ...
            this.substrateIndexs_unmodifiedNucleobases = this.substrateIndexs({'ADE';'CSN';'GUA';'THY'});
            this.substrateIndexs_unmodifiedNucleosides = this.substrateIndexs({'DAD_2'; 'DCYT'; 'DGSN'; 'THYMD'});
    ...
    methods
        ...
        function nVulnerableSites = calcNumberVulnerableSites(this)
			for i = 1:16
            c = this.chromosome;
            polDNA{i} = collapse(c.polymerizedRegions{i});
            bases = 'ACGT';
            dntpComposition{i} = [
            ...
            [posStrnds{i}, dmg{i}] = find(c.damagedSites{i}); %safely ignores m6AD because this process doesn't produce or modify m6AD; this is asserted in testConstants in unit test
			nVulnerableSites = zeros(size(this.reactionStoichiometryMatrix, 2), 1);
            for j = 1:size(this.reactionStoichiometryMatrix, 2) %change for i to j
                %% damage would be to undamaged sequence
                if ischar(this.reactionVulnerableMotifs{j})
                    nVulnerableSites(j) = polDNA{i} * prod(dntpComposition{i}(ismembc2(this.reactionVulnerableMotifs{j}, bases)));
                    continue;
                end
                ...
                [spcPosStrmds, spcDmg] = find(c.(this.reactionVulnerableMotifTypes{j}));
                if isempty(spcPosStrmds)
                    continue;
                end
                
                spcIdxs = find(spcDmg == this.reactionVulnerableMotifs{j});
                if isempty(spcIdxs)
                    continue;
                end
                
                idxs = find(dmg == this.reactionVulnerableMotifs{j});
                if isempty(idxs)
                    continue;
                end
                
                nVulnerableSites(j) = sum(edu.jiangnan.fmme.util.SparseMat.ismember_subs(...
                    spcPosStrmds(spcIdxs, :), posStrnds{i}(idxs{i}, :), [c.sequenceLen(i) c.nCompartments]));
            end
			end
        end
        ...
        function evolveState(this)
            ...
            for i = 1:length(this.reactionWholeCellModelIDs)
				...
				for k = 1:16
					[positionsStrands{k}, sideEffects{k}] = this.chromosome.setSiteDamaged(k, ...
					...
					if isempty(positionsStrands{k})
						continue;
					end
                
                % use small molecule reactants, produce small molecule products
                this.substrates = this.substrates + ...
                    size(positionsStrands{k}, 1) * this.reactionSmallMoleculeStoichiometryMatrix(:,j);
                end
                %side effects
