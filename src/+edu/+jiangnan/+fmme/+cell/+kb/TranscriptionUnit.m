...
classdef TranscriptionUnit < edu.jiangnan.fmme.cell.kb.ssRNA
    properties
        ...
        geneCompartments = edu.jiangnan.fmme.cell.kb.Compartment.empty(0,0);
        
        compartment      = edu.jiangnan.fmme.cell.kb.Compartment.empty(0,0);
        
        transcriptionFactorProteinMonomers            = edu.jiangnan.fmme.cell.kb.ProteinMonomer.empty(0,0);        
        ...
        transcriptionFactorProteinMonomerCompartments = edu.jiangnan.fmme.cell.kb.Compartment.empty(0,0);
        
        transcriptionFactorProteinComplexs            = edu.jiangnan.fmme.cell.kb.ProteinComplex.empty(0,0);        
        ...
        transcriptionFactorProteinComplexCompartments = edu.jiangnan.fmme.cell.kb.Compartment.empty(0,0);
    end
    ...
    methods
        function this = TranscriptionUnit(knowledgeBase, wid,wholeCellModelID,name,...
                ...
                tssCoordinate, dnaSequence,...
                comments,crossReferences)
            
            if nargin == 0; return; end;
            
            this = edu.jiangnan.fmme.cell.kb.TranscriptionUnit.empty(size(wid,1),0);
            this(size(wid,1),1) = edu.jiangnan.fmme.cell.kb.TranscriptionUnit;
            for i = 1:size(wid,1)
                this(i,1).idx = i;
        ...
        function value = get.synthesisRate(this)
            ...
            %compute
            value = mean(reshape([this.genes.synthesisRate],[],this.numGenes),1)';
        ...
        function value = get.promoter1Sequence(this)
            ...
            if this.direction
                ...
                value = seqcomplement(this.genomes.sequence(this.fivePrimeCoordinate - this.tssCoordinate + (1:-1:1) - 1));
            end
        ...
        %}
        function value = get.TATASequence(this)
            %retrieve
            if ~isempty(this.TATASequence)
                value = this.TATASequence;
                return;
            end
            
            %compute
            if this.direction
                value = this.genome.sequence(this.fivePrimeCoordinate + this.TATACoordinate + (-this.TATALength:-1) + 1);
            else
                value = seqcomplement(this.genome.sequence(this.fivePrimeCoordinate - this.TATACoordinate + (this.TATALength:-1:1) - 1));
            end
            
            %store
            this.TATASequence = value;
        end
        %{
        ...
        %}
         function value = get.CCAATSequence(this)
            %retrieve
            if ~isempty(this.CCAATSequence)
                value = this.CCAATSequence;
                return;
            end
            
            %compute
            if this.direction
                value = this.genome.sequence(this.fivePrimeCoordinate + this.CCAATCoordinate + (-this.CCAATLength:-1) + 1);
            else
                value = seqcomplement(this.genome.sequence(this.fivePrimeCoordinate - this.CCAATCoordinate + (this.CCAATLength:-1:1) - 1));
            end
            
            %store
            this.CCAATSequence = value;
         end 
         function value = get.GCSequence(this)
            %retrieve
            if ~isempty(this.GCSequence)
                value = this.GCSequence;
                return;
            end
            
            %compute
            if this.direction
                value = this.genome.sequence(this.fivePrimeCoordinate + this.GCCoordinate + (-this.GCLength:-1) + 1);
            else
                value = seqcomplement(this.genome.sequence(this.fivePrimeCoordinate - this.GCCoordinate + (this.GCLength:-1:1) - 1));
            end
            
            %store
            this.GCSequence = value;
         end 
