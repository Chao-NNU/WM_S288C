...
classdef SparseMat
        ...
        function this = subsasgn(this, s, rhs)
            ...
            if numel(s.subs)==1
                ...
                for i = 1:numel(lhsDims)
                    ...
                    if isscalar(subs{n}) || edu.jiangnan.fmme.util.SetUtil.isunique(subs{n})
    ...
    methods
        ...
        function C = tprod(A, B, Aid, Bid)
            ...
            %return as SparseMat
            if ~isa(A,'SparseMat') || (isa(B,'SparseMat') && ~strcmp(class(B),'edu.jiangnan.fmme.util.SparseMat'))
                C = B;
            else
                C = A;
            end
    ...
    methods
        ...
        function C = sum(A, dim)
            ...
            if isa(A, 'edu.jiangnan.fmme.util.CircularSparseMat')
                oldDims = 1:A.ndims;
                oldDims(dim) = [];
    ...
    methods (Static)
        function [subs2, I, J] = unique_subs(subs, siz)
            ...
            if nargout >= 3
                [~, J] = edu.jiangnan.fmme.util.SparseMat.ismember_subs(subs, subs2, siz);
            end
        end
